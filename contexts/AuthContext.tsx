...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    }, process.env.NODE_ENV === 'production' ? 18_000 : 12_000)
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      try {
        ...
        const GET_SESSION_MS = process.env.NODE_ENV === 'production' ? 15_000 : 10_000
        ...
        if (mounted) {
          ...
          if (currentSession?.user) {
            ...
            try {
              ...
              const withTimeout = <T,>(p: Promise<T | null>, ms: number) =>
                Promise.race([
                  p,
                  new Promise<null>((resolve) => setTimeout(() => resolve(null), ms)),
                ])

              // Fetch profile fresh (no cache) — bound wait so hung PostgREST can't block auth forever
              let userProfile = await withTimeout(
              ...
              if (!userProfile) {
                userProfile = await withTimeout(ensureProfileExists(currentSession.user), PROFILE_MS)
              }
