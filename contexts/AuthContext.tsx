...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Maximum timeout: 5 seconds - give enough time for OAuth redirects to start
    // CRITICAL: Don't timeout too fast or it might interrupt OAuth flow
    const globalTimeout = setTimeout(() => {
      if (mounted) {
        console.warn('⚠️ Auth check timed out after 5 seconds - marking as resolved')
        setLoading(false)
