...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const ensureProfileExists = useCallback(async (user: User) => {
    try {
      ...
      // (Database trigger might create it simultaneously)
      ...
      if (error) {
        // If it's a duplicate key error, that's okay - trigger might have created it
        if (error.code === '23505' || error.message?.includes('duplicate')) {
          console.log('ℹ️ Profile already exists (likely created by trigger) - fetching it')
          // Try to fetch it
          const existingProfile = await fetchUserProfile(user.id)
          if (existingProfile) {
            return existingProfile
      ...
      if (currentSession?.user) {
        ...
        if (event === 'SIGNED_IN') {
          ...
          try {
            ...
            // If profile doesn't exist, create it using upsert (handles race conditions)
            if (fetchError?.code === 'PGRST116' || !existingProfile) {
              console.log('🆕 [AUTH] Profile does not exist - creating profile with upsert')
              ...
              if (createError) {
                // If duplicate, profile was created by trigger - fetch it
                if (createError.code === '23505' || createError.message?.includes('duplicate')) {
                  console.log('ℹ️ [AUTH] Profile already exists (likely created by trigger) - fetching it')
