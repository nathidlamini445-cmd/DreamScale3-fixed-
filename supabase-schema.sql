...
-- Extracts name and email from Google OAuth metadata
...
DECLARE
  ...
  user_email TEXT;
BEGIN
  -- Extract email (always available from auth.users.email)
  user_email := NEW.email;
  
  -- Extract name from Google OAuth metadata
  -- Google provides name in various fields, check in order of preference
  user_name := COALESCE(
    NEW.raw_user_meta_data->>'full_name',      -- Google OAuth full_name
    NEW.raw_user_meta_data->>'name',           -- Google OAuth name
    NEW.user_metadata->>'full_name',            -- Alternative location
    NEW.user_metadata->>'name',                 -- Alternative location
    NULL
  );
