...
import { createDataProxyClient } from './supabase/data-proxy-client'
import type { User } from '@supabase/supabase-js'

// Clerk (not Supabase Auth) is the auth source, so the anon browser client has no
// Supabase session and RLS (`auth.uid()::text = user_id`) silently blocks every read/write.
// Route all data access through the service-role-backed `/api/db` proxy, which scopes
// every operation to the authenticated Clerk user id. See lib/supabase/data-proxy-client.ts.
...
export async function saveSessionData(userId: string, sessionData: any): Promise<void> {
  try {
    if (typeof window === 'undefined') {
      throw new Error('saveSessionData must run in the browser')
    }
    ...
    if (!res.ok) {
      const msg = [payload.error, payload.hint].filter(Boolean).join(' — ')
      throw new Error(msg || `Save session failed (${res.status})`)
    }
  } catch (error) {
...
export async function loadSessionData(userId: string): Promise<any | null> {
  const USER_SESSION_FETCH_MS = 20_000

  try {
    if (typeof window === 'undefined') {
      throw new Error('loadSessionData must run in the browser')
    }

    const controller = new AbortController()
    const timer = setTimeout(() => controller.abort(), USER_SESSION_FETCH_MS)
    ...
    if (res.status === 401) {
      throw new Error('Unauthorized loading session')
    }
    ...
    if (!res.ok) {
      const msg = [payload.error, payload.hint].filter(Boolean).join(' — ')
      throw new Error(msg || `Load session failed (${res.status})`)
    }

    return (payload.sessionData as Record<string, unknown> | null) ?? null
  } catch (e: unknown) {
    const name = e instanceof Error ? e.name : ''
    if (name === 'AbortError') {
      throw new Error('Supabase data load timeout')
    }
