...
function checkForUpdates(feature: keyof FeatureNotifications): boolean {
  ...
  switch (feature) {
    ...
    case 'flowmatch': // Systems
      // Check for auto-setup systems
      if (typeof window !== 'undefined') {
        const systems = localStorage.getItem('systembuilder:systems')
        if (systems) {
          try {
            const systemsArray = JSON.parse(systems)
            return Array.isArray(systemsArray) && systemsArray.length > 0
          } catch (e) {
            return false
          }
        }
    ...
    case 'revenue':
      // Check for auto-setup revenue content (goals, optimizations, pricing strategies, dashboards)
      if (typeof window !== 'undefined') {
        const revenueData = localStorage.getItem('revenueos:data')
        if (revenueData) {
          try {
            const data = JSON.parse(revenueData)
            const hasGoals = Array.isArray(data.goals) && data.goals.length > 0
            const hasOptimizations = Array.isArray(data.optimizations) && data.optimizations.length > 0
            const hasPricingStrategies = Array.isArray(data.pricingStrategies) && data.pricingStrategies.length > 0
            const hasDashboards = Array.isArray(data.dashboards) && data.dashboards.length > 0
            return hasGoals || hasOptimizations || hasPricingStrategies || hasDashboards
          } catch (e) {
            return false
          }
        }
    ...
    case 'pitchpoint': // Leadership
      // Check for auto-setup leadership content (decisions, communications, conflicts, routines)
      if (typeof window !== 'undefined') {
        const leadershipData = localStorage.getItem('leadership:data')
        if (leadershipData) {
          try {
            const data = JSON.parse(leadershipData)
            const hasDecisions = Array.isArray(data.decisions) && data.decisions.length > 0
            const hasCommunications = Array.isArray(data.communications) && data.communications.length > 0
            const hasConflicts = Array.isArray(data.conflicts) && data.conflicts.length > 0
            const hasRoutines = Array.isArray(data.routines) && data.routines.length > 0
            return hasDecisions || hasCommunications || hasConflicts || hasRoutines
          } catch (e) {
            return false
          }
        }
    ...
    case 'teams':
      // Check for auto-setup team content (members, rituals, dnaAnalyses, healthMonitors)
      if (typeof window !== 'undefined') {
        ...
        if (teamsData) {
          try {
            ...
            const hasRituals = Array.isArray(data.rituals) && data.rituals.length > 0
            const hasDNA = Array.isArray(data.dnaAnalyses) && data.dnaAnalyses.length > 0
            const hasHealthMonitors = Array.isArray(data.healthMonitors) && data.healthMonitors.length > 0
            return hasMembers || hasRituals || hasDNA || hasHealthMonitors
          } catch (e) {
            return false
          }
        }
