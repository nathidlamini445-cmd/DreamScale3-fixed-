export interface PDFData {
  ...
  companyName?: string
  /**
   * Analysis or generation date for the PDF. Optional so non-competitor flows
   * (SOPs, generic exports) can omit it; legacy callers may also use `date`.
   */
  analysisDate?: string
  date?: string
        ...
        <div class="header">
            ...
            <div class="company-info">
                <div class="left">
                    ...
                    ${data.competitorUrl ? `<div class="url-badge">${data.competitorUrl}</div>` : ''}
                </div>
                <div class="right">
                    ${data.analysisDate || data.date ? `<div><strong>Analysis Date:</strong> ${data.analysisDate || data.date}</div>` : ''}
...
function formatContentForPDF(content: string): string {
  ...
  // Wrap consecutive list items in ul/ol tags. The `g` + `s` flags both
  // need ES2018+ — that's now configured in tsconfig.json.
...
export async function downloadPDF(data: PDFData, filename?: string): Promise<void> {
  try {
    ...
    console.log('Starting PDF generation...', {
      ...
      hasContent: (contentDiv.textContent?.trim().length ?? 0) > 0,
    })
