    ...
    for (const catKey of categoriesToFetch) {
      ...
      try {
        ...
        feed.items.forEach((item: any) => {
          ...
          // Deterministic "match %" derived from the job link so the value is
          // stable across re-renders (was Math.random() which flickered on
          // every re-render).
          const stableId = item.link || item.title || ''
          let hash = 0
          for (let i = 0; i < stableId.length; i++) {
            hash = ((hash << 5) - hash + stableId.charCodeAt(i)) | 0
          }
          const matchPercentage = 70 + (Math.abs(hash) % 31) // 70–100, deterministic
...
export async function searchJobs(keyword: string, limit = 30): Promise<UpworkResponse> {
  try {
    ...
    const jobs: UpworkJob[] = feed.items.map((item: any) => {
      const cleanDescription = item.description
        ?.replace(/<[^>]*>/g, '')
        .replace(/&nbsp;/g, ' ')
        .replace(/&amp;/g, '&')
