...
/** Parallel Supabase reads can be slow on cold start / long RTT; too-aggressive timeouts caused empty sessions + reload storms. */
      ...
      if (userId) {
        try {
          ...
          const loadDataPromise = Promise.all([
            ...
            supabaseData.loadDailyMood(userId, new Date().toISOString().split('T')[0]),
          ...
          try {
            ;[
              ...
              dailyMood,
              tasksData
            ] = (await Promise.race([loadDataPromise, timeoutPromise])) as any
            if (timeoutId !== undefined) clearTimeout(timeoutId)
          } catch (timeoutError) {
            if (timeoutId !== undefined) clearTimeout(timeoutId)
            console.warn(
              `⚠️ Supabase data load timed out after ${SUPABASE_LOAD_TIMEOUT_MS / 1000}s — starting a fresh in-memory session; data will sync when the API responds`
            )
            throw timeoutError
          }
          ...
          if (gen === loadGenRef.current) {
            ...
            setIsSessionActive(false) // Do not set true on error — save effect would run → re-load → loop
