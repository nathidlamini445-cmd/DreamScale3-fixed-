...
/** Timeout for loading the critical `user_sessions` row only (extended data hydrates in background). */
const SUPABASE_LOAD_TIMEOUT_MS = 12_000
...
export function SessionProvider({ children }: { children: React.ReactNode }) {
    ...
    try {
      ...
      if (userId) {
        try {
          ...
          // Phase 1: user_sessions only — was blocked by Promise.all waiting on the slowest table.
          let criticalTimer: ReturnType<typeof setTimeout> | undefined
          const criticalTimeout = new Promise<never>((_, reject) => {
          ...
          try {
            ...
            if (criticalTimer !== undefined) clearTimeout(criticalTimer)
          } catch (timeoutErr) {
            if (criticalTimer !== undefined) clearTimeout(criticalTimer)
            supabaseParallelLoadBlockedUntilRef.current = Date.now() + 60_000
            console.warn(
              `⚠️ Supabase core session timed out after ${SUPABASE_LOAD_TIMEOUT_MS / 1000}s`
            )
            throw timeoutErr
          }
   ...
   */
  useEffect(() => {
    if (!mounted || !userId || !user?.email) return
  ...
  }, [mounted, userId, user?.email, loadUserData])
