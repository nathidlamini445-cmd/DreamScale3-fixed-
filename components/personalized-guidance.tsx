...
export function PersonalizedGuidance() {
  ...
  // Normalize the entrepreneur profile so multi-select fields (industry / businessStage)
  // are coerced to the single-string types expected by the content helpers.
  const normalizedProfile = entrepreneurProfile
    ? {
        ...entrepreneurProfile,
        industry: Array.isArray(entrepreneurProfile.industry)
          ? entrepreneurProfile.industry[0] ?? null
          : entrepreneurProfile.industry,
        businessStage: ((): 'idea' | 'foundation' | 'established' | null => {
          const raw = Array.isArray(entrepreneurProfile.businessStage)
            ? entrepreneurProfile.businessStage[0]
            : entrepreneurProfile.businessStage
          return raw === 'idea' || raw === 'foundation' || raw === 'established' ? raw : null
        })(),
      }
    : null

  const guidance = normalizedProfile ? getPersonalizedGuidance(normalizedProfile) : null
  const nextSteps = normalizedProfile ? getNextSteps(normalizedProfile) : []
  const mindsetQuestions = normalizedProfile ? getMindsetQuestions(normalizedProfile) : []
