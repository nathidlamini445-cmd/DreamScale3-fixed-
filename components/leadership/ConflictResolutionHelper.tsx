            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  onClick={() => {
                    if (selectedConflict) {
                      let content = `# Conflict Resolution Guidance\n\n`
                      content += `**Situation:** ${selectedConflict.situation}\n`
                      content += `**Parties:** ${selectedConflict.parties.join(', ')}\n\n`
                      content += `## Conversation Structure\n\n`
                      selectedConflict.guidance.conversationStructure.forEach((step, index) => {
                        content += `${index + 1}. ${step}\n`
                      })
                      content += `\n## Step-by-Step Action Plan\n\n`
                      selectedConflict.guidance.steps.forEach(step => {
                        content += `### Step ${step.step}: ${step.action}\n\n${step.script}\n\n`
                      })
                      if (selectedConflict.guidance.scripts.length > 0) {
                        content += `## Conversation Scripts\n\n`
                        selectedConflict.guidance.scripts.forEach((script, index) => {
