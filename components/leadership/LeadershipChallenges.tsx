...
export default function LeadershipChallenges({ challenges, onUpdateChallenge, onDeleteChallenge }: LeadershipChallengesProps) {
  ...
  useEffect(() => {
    if (navigatingId) {
      ...
      if (pathname && pathname.startsWith('/leadership/challenge/')) {
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          setNavigatingId(challenge.id)
                          router.push(`/leadership/challenge/${challenge.id}`)
                        }}
                        disabled={navigatingId === challenge.id}
                      >
                        {navigatingId === challenge.id ? (
                          <>
                            <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                            Opening...
                          </>
