...
export default function LeadershipStyleAssessment({ data, onComplete }: LeadershipStyleAssessmentProps) {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>Your Leadership Style</CardTitle>
              <CardDescription>Assessment completed on {new Date(results.date).toLocaleDateString()}</CardDescription>
            </div>
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Leadership Style Assessment</CardTitle>
        <CardDescription>
          ...
          <div className="space-y-3">
            {ASSESSMENT_QUESTIONS[currentQuestion].options.map((option, index) => (
              <Button
                key={index}
                variant="outline"
                className="w-full justify-start h-auto py-4 px-4 text-left hover:bg-[#39d2c0]/10 hover:border-[#39d2c0]"
