...
export function AIResponse({ content, onCopy, onLike, onDislike, onShare, onAddToPrompt }: AIResponseProps) {
  ...
  const components = {
    ...
    li: ({ children, ...props }: any) => {
      ...
      // For unordered lists - make bullet points more prominent
      return (
            <li className="flex items-start gap-4 text-xl font-medium text-black dark:text-gray-200" style={{ fontFamily: '"Libre Baskerville", serif' }}>
          <span className="text-[#39d2c0] dark:text-[#39d2c0] mt-1 text-2xl font-bold leading-none" style={{ textShadow: '0 1px 2px rgba(57, 210, 192, 0.3), 0 2px 4px rgba(57, 210, 192, 0.2)' }}>•</span>
          <span className="flex-1 leading-relaxed" style={{ textShadow: '0 1px 1px rgba(0, 0, 0, 0.04)' }}>{children}</span>
        </li>
      )
    },
    strong: ({ children }: any) => (
      <strong className="font-bold text-[#39d2c0] dark:text-[#39d2c0]" style={{ fontFamily: '"Libre Baskerville", serif', textShadow: '0 1px 1px rgba(0, 0, 0, 0.15)' }}>
        {children}
      </strong>
    ),
    em: ({ children }: any) => (
