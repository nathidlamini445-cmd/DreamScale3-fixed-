...
export function AIResponse({ content, onCopy, onLike, onDislike, onShare, onAddToPrompt }: AIResponseProps) {
  ...
  const components = {
    ...
    p: ({ children }: any) => (
      <p className="text-xl font-medium text-black dark:text-gray-200 mb-5 leading-relaxed" style={{ fontFamily: '"Libre Baskerville", serif', textShadow: '0 1px 1px rgba(0, 0, 0, 0.04)', willChange: 'auto', contain: 'layout style paint' }}>
        {children}
      </p>
    ),
    ul: ({ children }: any) => (
    ...
    li: ({ children, ...props }: any) => {
      ...
      // For unordered lists - make bullet points more prominent with gradient (no animation for performance)
      return (
            <li className="flex items-start gap-4 text-xl font-medium text-black dark:text-gray-200" style={{ fontFamily: '"Libre Baskerville", serif', willChange: 'auto', contain: 'layout style paint' }}>
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#005DFF] to-cyan-300 dark:from-[#005DFF] dark:to-[#39d2c0] mt-1 text-2xl font-bold leading-none inline-block" style={{ textShadow: '0 1px 2px rgba(0, 93, 255, 0.3), 0 2px 4px rgba(0, 93, 255, 0.2)', willChange: 'auto', contain: 'layout style paint' }}>
            •
          </span>
          <span className="flex-1 leading-relaxed" style={{ textShadow: '0 1px 1px rgba(0, 0, 0, 0.04)', willChange: 'auto' }}>{children}</span>
        </li>
      )
    },
    strong: ({ children }: any) => (
      <strong className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#005DFF] to-cyan-300 dark:from-[#005DFF] dark:to-[#39d2c0] inline align-baseline" style={{ fontFamily: '"Libre Baskerville", serif', textShadow: '0 1px 1px rgba(0, 93, 255, 0.15)', verticalAlign: 'baseline', willChange: 'auto', contain: 'layout style paint' }}>
        {children}
      </strong>
    ),
    em: ({ children }: any) => (
    ...
    a: ({ children, href }: any) => (
      <a 
        ...
        className="text-[#005DFF] dark:text-blue-400 hover:text-[#0048CC] dark:hover:text-blue-300 underline underline-offset-2 transition-colors inline align-baseline"
