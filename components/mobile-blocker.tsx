...
export function MobileBlocker({ children }: { children: React.ReactNode }) {
  ...
  const isAllowedRoute = MOBILE_ALLOWED_ROUTES.some(
  ...
  useEffect(() => {
    ...
    // Throttle the resize handler to one update per animation frame so we
    // don't re-render on every pixel during a window drag.
    const onResize = () => {
      if (resizeRaf) return
