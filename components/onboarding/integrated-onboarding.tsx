...
import { generateTasksFromOnboardingAI } from '@/lib/tasks/generate-tasks-from-onboarding'
...
const entrepreneurImages = [
  ...
  },
  {
    ...
    title: 'CEO, Tesla & X',
    position: 'bottom-right',
    fallback: '/Elon musk. story. 07-12.jpg' // Fallback to story version
  },
  {
    ...
    position: 'bottom-right',
    fallback: '/Jeff Bezos. Story. 07-12.jpg' // Fallback to story version
  },
  {
    ...
    title: 'CEO, NVIDIA',
    position: 'bottom-right',
    fallback: '/Jensen Huang. Story. 07-12.jpg' // Fallback to story version
  },
  {
    ...
    title: 'CEO, Meta',
    position: 'bottom-right',
    fallback: '/Mark Zukerberg. Story. 07-12.jpg' // Fallback to story version (note: different spelling in filename)
  },
  {
    ...
    position: 'bottom-right',
    fallback: '/Emma-Grede. Story 07-12.jpg' // Fallback to story version
  },
  {
    ...
    position: 'bottom-right',
    fallback: '/Robert F smith. Story. 07-12.jpg' // Fallback to story version
  },
  {
    ...
    title: 'CEO, Microsoft',
    position: 'bottom-right',
    fallback: '/Satya-Nadella-Story-07-12.jpg' // Fallback to story version
  },
  {
    ...
    title: 'Founder, Spanx',
    position: 'bottom-right',
    fallback: '/Sara-Blakely-Story-07-12.jpg' // Fallback to story version
  }
]
...
export function IntegratedOnboarding({ isOpen, onClose }: IntegratedOnboardingProps) {
  ...
  // Helper function to normalize array values for database
  const normalizeArrayForDB = (value: string | string[] | undefined): string[] => {
    if (!value) return []
  ...
  // Helper function to normalize text values for database
  const normalizeTextForDB = (value: string | string[] | undefined): string | null => {
    if (!value) return null
  ...
  const handleSubmit = async (data: OnboardingData) => {
    ...
    try {
      ...
      const userEmail = user?.email || sessionContext?.sessionData?.email || 
                        (typeof window !== 'undefined' ? localStorage.getItem('dreamscale_current_email') : null)

      // If user is not authenticated, save to localStorage and session context
      // Then redirect to login - login page will check onboarding_complete and redirect to home
      if (!user?.id) {
        console.log('💾 Saving onboarding data to localStorage (user not authenticated yet)')
        ...
        if (typeof window !== 'undefined' && userEmail) {
          try {
            const existingData = localStorage.getItem(`dreamscale_session_${userEmail}`)
            ...
            localStorage.setItem(`dreamscale_session_${userEmail}`, JSON.stringify(updatedData))
            localStorage.setItem('dreamscale_onboarding_completed', 'true')
        ...
        // Redirect to login with onboarding_complete flag - login page will redirect to home
        setTimeout(() => {
          if (typeof window !== 'undefined') {
            console.log('🔄 [ONBOARDING] Redirecting to login with onboarding_complete=true')
            window.location.href = '/login?onboarding_complete=true'
          }
        }, 1500)
        return
      }
      ...
      console.log('💾 Saving ALL onboarding data to Supabase user_profiles for user:', user.id)
      
      // Prepare ALL onboarding data for Supabase user_profiles table
      const profileUpdate = {
        // Basic info
        full_name: normalizeTextForDB(data.name) || user.user_metadata?.full_name || null,
        user_name: normalizeTextForDB(data.name) || null,
        
        // Business info
        business_name: normalizeTextForDB(data.businessName) || null,
        industry: normalizeArrayForDB(data.industry),
        business_stage: normalizeTextForDB(data.businessStage) || null,
        revenue_goal: normalizeTextForDB(data.revenueGoal) || null,
        target_market: normalizeArrayForDB(data.targetMarket),
        team_size: normalizeTextForDB(data.teamSize) || null,
        biggest_challenges: normalizeArrayForDB(data.challenges),
        
        // Revenue & growth
        revenue_model: normalizeTextForDB(data.primaryRevenue) || null,
        customer_acquisition: normalizeArrayForDB(data.customerAcquisition),
        mrr: normalizeTextForDB(data.monthlyRevenue) || null,
        key_metrics: normalizeArrayForDB(data.keyMetrics),
        growth_strategy: normalizeTextForDB(data.growthStrategy) || null,
        six_month_goal: normalizeTextForDB(data.biggestGoal) || null,
        
        // Personal
        hobbies: normalizeArrayForDB(data.hobbies),
        favorite_song: normalizeTextForDB(data.favoriteSong) || null,
        
        // Completion flag - CRITICAL: Must be true
        onboarding_completed: true,
        updated_at: new Date().toISOString()
      }

      // CRITICAL: Save ALL onboarding data to user_profiles table using upsert
      // This MUST succeed for onboarding to be considered complete
      ...
      if (profileError) {
        console.error('❌ Error saving onboarding data to Supabase:', profileError)
        throw new Error(`Failed to save onboarding: ${profileError.message}`)
      }

      if (!updatedProfile) {
        throw new Error('Failed to save onboarding: No profile returned from Supabase')
      }
      
      // CRITICAL: Verify onboarding_completed was saved correctly
      if (updatedProfile.onboarding_completed !== true) {
        console.error('❌ CRITICAL: onboarding_completed was not set to true!', updatedProfile)
        // Force update it
        ...
        if (fixError) {
          throw new Error(`Failed to set onboarding_completed: ${fixError.message}`)
        }
        console.log('✅ Fixed onboarding_completed flag')
      }
      
      // CRITICAL: Clear profile cache to force fresh fetch on next login
      if (typeof window !== 'undefined') {
        try {
          sessionStorage.removeItem(`profile_${user.id}`)
          // Also clear any other profile caches
          const keys = Object.keys(sessionStorage)
          keys.forEach(key => {
            if (key.startsWith(`profile_${user.id}`)) {
      ...
      console.log('✅ ALL onboarding data saved to Supabase user_profiles:', {
        userId: user.id,
      ...
      toast({
        title: 'Onboarding Completed!',
        description: 'Your profile has been saved successfully. Generating your personalized tasks...',
        variant: 'default',
      })

      // CRITICAL: Save entrepreneurProfile to Supabase user_sessions table
      if (user?.id) {
        try {
          await supabaseData.saveSessionData(user.id, {
            entrepreneurProfile: {
              ...sessionData,
              onboardingCompleted: true
            }
      ...
      // CRITICAL: Save onboarding completion to localStorage for fast checks
      if (typeof window !== 'undefined') {
        try {
          localStorage.setItem('dreamscale_onboarding_completed', 'true')
          console.log('✅ Saved onboarding completion to localStorage')
        } catch (e) {
          console.warn('Failed to save onboarding completion to localStorage:', e)
        }
      }
      
      // Update session context with onboarding completion
      if (sessionContext) {
        sessionContext.updateEntrepreneurProfile({
          ...sessionData,
          onboardingCompleted: true
      ...
      // CRITICAL: Generate tasks after onboarding and save to Supabase
      console.log('🎯 Generating tasks from onboarding data...')
      try {
        const generatedTasks = await generateTasksFromOnboardingAI(data)
        console.log('✅ Tasks generated:', {
          daily: generatedTasks.daily.length,
          weekly: generatedTasks.weekly.length,
          monthly: generatedTasks.monthly.length,
          yearly: generatedTasks.yearly.length
        })
        
        // Save tasks to Supabase
        if (user?.id) {
          await supabaseData.saveTasksData(user.id, generatedTasks)
          console.log('✅ Tasks saved to Supabase')
          
          // Also update session context IMMEDIATELY so TasksPreview can see them
          if (sessionContext?.updateTasksData) {
            sessionContext.updateTasksData(generatedTasks)
            console.log('✅ Tasks updated in session context')
          }
        }
      } catch (error) {
        console.error('⚠️ Error generating tasks (will be generated on tasks page):', error)
        // Don't block onboarding completion if task generation fails
        toast({
          title: 'Tasks Generation Delayed',
          description: 'Your profile was saved, but task generation will happen on the tasks page.',
          variant: 'default',
        })
      }
      ...
      // CRITICAL: Redirect through /auth/resolve to ensure profile is refreshed
      // This ensures the auth system recognizes onboarding is complete
      // Wait a moment for database update to propagate
      if (typeof window !== 'undefined') {
        setTimeout(() => {
          console.log('🔄 [ONBOARDING] Redirecting to /auth/resolve to refresh auth state')
          console.log('🔄 [ONBOARDING] Using MULTIPLE redirect methods to ensure it works')
          ...
          setTimeout(() => {
            window.location.assign('/auth/resolve')
          }, 150)
        }, 1000) // Reduced delay since we're going through auth/resolve
      } else {
        setTimeout(() => {
          onClose()
          router.replace('/auth/resolve')
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white dark:bg-slate-950 overflow-y-auto">
      {/* Logo - Top Left */}
      <div className="fixed top-4 left-10 z-50 pointer-events-auto">
        <Image
          src="/Logo.png"
          alt="DreamScale Logo"
          width={120}
      ...
      {/* Sign-in (only before Google session — avoids duplicate Sign Up / Login noise) */}
      <div className="fixed top-4 right-4 z-[1000] pointer-events-auto sm:right-10" style={{ zIndex: 1000 }}>
        {!user?.id ? <LoginButton /> : (
          <p className="max-w-[200px] text-right text-xs text-gray-600 dark:text-gray-400 sm:max-w-xs">
            Signed in — continue below.
          </p>
        )}
      </div>
