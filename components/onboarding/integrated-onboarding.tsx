...
export function IntegratedOnboarding({ isOpen, onClose }: IntegratedOnboardingProps) {
  ...
  const handleSubmit = async (data: OnboardingData) => {
    ...
    try {
      // CRITICAL: User must be authenticated to complete onboarding
      if (!user?.id) {
      ...
      // Wait a bit longer to ensure database update has propagated
      // Then redirect directly to dashboard since we know onboarding is complete
      if (typeof window !== 'undefined') {
        // Longer delay to ensure DB update propagates, then redirect to dashboard
        setTimeout(() => {
