...
export default function ProcessDocumentation({ systems }: ProcessDocumentationProps) {
  ...
  // Load saved SOPs from Supabase (if authenticated) or localStorage - reload every time component mounts
  useEffect(() => {
    const loadSOPs = async () => {
      ...
      if (user?.id) {
        try {
          ...
          if (systemsData?.savedSOPs && systemsData.savedSOPs.length > 0) {
            const withDates = systemsData.savedSOPs.map((sop: any) => ({
              ...sop,
              timestamp: typeof sop.timestamp === 'string' ? new Date(sop.timestamp) : sop.timestamp
            }))
            setSavedSOPs(withDates)
            console.log('✅ Loaded SOPs from Supabase:', withDates.length)
            setTimeout(() => {
              isSavingRef.current = false
            }, 200)
      ...
      if (sessionContext?.sessionData?.systems?.savedSOPs) {
        ...
        console.log('✅ Loaded SOPs from session context:', deserialized.length)
      }
  ...
  // Save SOPs to Supabase (if authenticated) or localStorage
  useEffect(() => {
    ...
    const saveSOPs = async () => {
      ...
      // IMPORTANT: Only pass savedSOPs, not systems, to preserve existing systems
      if (user?.id) {
        try {
          await supabaseData.saveSystemsData(user.id, {
            savedSOPs: sopsToSave
            // Don't pass systems - let saveSystemsData load and preserve existing systems
          })
          console.log('✅ Saved SOPs to Supabase:', sopsToSave.length)
        } catch (error) {
      ...
      if (!user?.id && typeof window !== 'undefined') {
        try {
          ...
          console.log('💾 Saved SOPs to localStorage (unauthenticated):', sopsToSave.length)
        } catch (e) {
  ...
  useEffect(() => {
    ...
    if (sessionContext?.updateSystemsData && savedSOPs.length >= 0) {
      ...
      sessionContext.updateSystemsData({
        savedSOPs: savedSOPs
        // Don't pass systems - let updateSystemsData preserve existing systems
      })
      setTimeout(() => {
        isSavingRef.current = false
      }, 100)
    ...
    try {
      ...
      if (user?.id) {
        try {
          ...
          // CRITICAL: Only pass savedSOPs, NOT systems
          // This preserves existing systems in Supabase
          console.log('💾 Saving SOP to Supabase (preserving existing systems)...', {
            sopsCount: sopsToSave.length,
            notPassingSystems: true
          })
          
          await supabaseData.saveSystemsData(user.id, {
            // DO NOT pass systems: [] - this would overwrite existing systems!
            // Only pass savedSOPs - saveSystemsData will preserve existing systems
            savedSOPs: sopsToSave
          })
          console.log('✅ Saved SOP to Supabase:', newSOP.title)
        } catch (e) {
          console.warn('Failed to save SOP to Supabase:', e)
        }
      }
      ...
      if (!user?.id && typeof window !== 'undefined') {
        try {
          ...
          console.log('✅ Saved SOP to localStorage (unauthenticated):', newSOP.title)
        } catch (e) {
