...
export default function SystemTemplates({ onSelectTemplate }: SystemTemplatesProps) {
  ...
  const handleTemplateSelect = (template: Template) => {
    // Call immediately without delay, including template name
    onSelectTemplate({
  ...
  const handleButtonClick = (e: React.MouseEvent, template: Template) => {
    ...
    setClickedTemplateId(template.id)
    // Call handler immediately
    handleTemplateSelect(template)
    // Reset after a short delay (in case of errors)
    setTimeout(() => setClickedTemplateId(null), 3000)
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {templates.map((template) => (
          <div
            ...
            onClick={(e) => {
              if (clickedTemplateId !== template.id) {
                setClickedTemplateId(template.id)
                handleTemplateSelect(template)
                setTimeout(() => setClickedTemplateId(null), 3000)
              }
          ...
          >
            ...
            <Button 
              ...
              disabled={clickedTemplateId === template.id}
            >
              {clickedTemplateId === template.id ? (
                <>
