...
import { Target, Flame, Zap, Trophy, ArrowRight, Clock, AlertTriangle } from 'lucide-react'
...
interface VentureQuestPanelProps {
  onClose: () => void
}

// Simple Onboarding Component for the panel
...
export default function VentureQuestPanel({ onClose }: VentureQuestPanelProps) {
  ...
  if (showOnboarding) {
    return <SimpleOnboardingWizard onComplete={handleOnboardingComplete} />
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className={`${streakInDanger ? 'border-2 border-orange-500 dark:border-orange-500' : 'border border-gray-200 dark:border-gray-800'}`}>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center gap-3">
              <Flame className={`w-8 h-8 ${streakInDanger ? 'text-orange-500' : 'text-orange-500'}`} />
              <div>
                <span className="text-4xl font-bold text-gray-900 dark:text-white">
                  {user?.currentStreak || 0}
                </span>
                <span className="text-base text-gray-500 ml-2">days</span>
              </div>
            </div>
            ...
            {user.goalProgress !== undefined && (
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-gray-900 dark:text-white">{user.goalProgress}%</span>
                </div>
                <Progress value={user.goalProgress} className="h-3" />
              </div>
            )}
          </CardContent>
        </Card>
