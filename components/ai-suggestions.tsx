...
function generateSmartSuggestions(onboardingData: OnboardingData | null): Suggestion[] {
  ...
  suggestions.push({
    ...
    explanation: `Every business is unique, and sometimes you have questions that don't fit into neat categories. Maybe you're wondering about a specific pricing decision, how to handle a difficult conversation, or whether a particular opportunity is worth pursuing. Bizora AI lets you ask any business question and get thoughtful, personalized answers based on your industry, stage, goals, and even your hobbies. It's like having a business advisor available 24/7 who actually knows your context and can relate business advice to what you already enjoy.`,
    feature: 'Bizora AI',
    featureLink: '/bizora',
    icon: 'compass',
    priority: 'medium',
...
export function AISuggestions() {
  ...
  const [seenRecommendationIds, setSeenRecommendationIds] = useState<Set<string>>(new Set())
  ...
  useEffect(() => {
    const loadSuggestions = async () => {
      ...
      // Only show cached recommendations - don't generate on home page
      // Generation will happen on the suggestions page when user clicks "View All Recommendations"
      const savedRecs = localStorage.getItem('homePageRecommendations')
      if (savedRecs) {
        try {
          const parsed = JSON.parse(savedRecs)
          // Check if cached recommendations have outdated "Discover" references that should be "Bizora AI"
          const hasOutdatedDiscover = parsed.some((rec: Suggestion) => 
            rec.explanation?.includes('Our Discover feature') || 
            (rec.feature === 'Discover' && rec.featureLink === '/discover' && rec.title === 'Get Answers to Your Specific Questions')
          )
          
          // If outdated cache found, clear it and regenerate
          if (hasOutdatedDiscover) {
            console.log('Clearing outdated recommendations cache')
            localStorage.removeItem('homePageRecommendations')
            // Fall through to generate fresh recommendations
          } else {
            // Ensure parsed is an array before using map
            if (Array.isArray(parsed) && parsed.length > 0) {
              setSuggestions(parsed)
              // Track initial recommendations we've seen
              const initialSeenIds = new Set(parsed.map((rec: Suggestion) => rec.id))
              setSeenRecommendationIds(initialSeenIds)
              setIsLoading(false)
              return
            } else {
              // Invalid cache format, clear it and regenerate
              localStorage.removeItem('homePageRecommendations')
            }
          }
        } catch (e) {
      ...
      // If no cached recommendations, show empty state or fallback
      // Don't generate here - let the suggestions page handle generation
      const generatedSuggestions = generateSmartSuggestions(data)
      const top3 = Array.isArray(generatedSuggestions) && generatedSuggestions.length > 0 
  ...
  const displayedSuggestions = Array.isArray(suggestions) && suggestions.length > 0 
    ? suggestions.slice(0, 3) 
    : []

  if (isLoading) {
    return (
      <Card className="p-6 border-gray-200/60 dark:border-gray-800/60 bg-white dark:bg-gray-900 shadow-[0_1px_2px_0_rgba(0,0,0,0.05)] dark:shadow-[0_1px_2px_0_rgba(0,0,0,0.3)]">
        <div className="mb-4">
          <h3 className="text-xl font-semibold text-foreground mb-1">What You Should Focus On</h3>
        </div>
        <div className="flex flex-col items-center justify-center py-10 space-y-5">
          ...
          <div className="flex items-center gap-1.5 text-xs text-gray-400 dark:text-gray-600 mt-2">
  ...
  return (
    <Card className="p-6 border-gray-200/60 dark:border-gray-800/60 bg-white dark:bg-gray-900 shadow-[0_1px_2px_0_rgba(0,0,0,0.05)] dark:shadow-[0_1px_2px_0_rgba(0,0,0,0.3)]">
      ...
      <div className="space-y-3 mb-4">
        {Array.isArray(displayedSuggestions) && displayedSuggestions.length > 0 && displayedSuggestions.map((suggestion) => {
      ...
      <div className="flex justify-center">
        <Link href="/suggestions?generate=true">
          <Button 
            variant="outline" 
