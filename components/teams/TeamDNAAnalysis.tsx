      ...
      {analyses.length > 0 && (
        <div className="space-y-6">
          <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
            Your Team DNA Analyses ({analyses.length})
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {analyses.map((analysis) => (
              <AnalysisItemCard
                ...
                subtitle={`${analysis.members.length} member${analysis.members.length !== 1 ? 's' : ''}`}
                date={analysis.date}
                icon={<Dna className="w-5 h-5 text-blue-600" />}
                badges={[
                  { label: `${analysis.members.length} members`, variant: 'secondary' }
                ]}
                onDelete={(id) => {
                  if (onDeleteAnalysis) {
                    onDeleteAnalysis(id)
