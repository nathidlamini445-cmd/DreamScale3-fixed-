...
export default function TeamRitualsBuilder({ rituals, onAddRitual, onDeleteRitual }: TeamRitualsBuilderProps) {
  ...
  const handleGenerateRitual = async () => {
    ...
    try {
      ...
      console.log('✅ Received ritual data:', ritualData)
      ...
      const ritual: TeamRitual = {
        ...
        structure: {
          sections: (ritualData.structure?.sections || []).map((section: any) => ({
            name: section.name || 'Section',
            duration: section.duration || 5,
            description: section.description || '',
            activities: section.activities || [],
            facilitation: section.facilitation || '',
            outcomes: section.outcomes || [],
            tips: section.tips || []
          }))
        },
        purpose: ritualData.purpose || 'Regular team ritual for alignment and collaboration',
        participants: Array.isArray(ritualData.participants) ? ritualData.participants : (ritualData.participants ? [ritualData.participants] : ['All team members']),
        ...
        aiSuggestions: {
          whyNeeded: ritualData.aiSuggestions?.whyNeeded || ritualData.aiSuggestions?.whyNeeded || 'This ritual helps maintain team alignment and communication.',
          bestPractices: ritualData.aiSuggestions?.bestPractices || ritualData.aiSuggestions?.bestPractices || ['Start on time', 'Keep it focused', 'Document action items'],
          commonMistakes: ritualData.aiSuggestions?.commonMistakes || ritualData.aiSuggestions?.commonMistakes || ['Going over time', 'Lack of follow-up'],
          adaptations: ritualData.aiSuggestions?.adaptations || [],
          successFactors: ritualData.aiSuggestions?.successFactors || [],
          measuringSuccess: ritualData.aiSuggestions?.measuringSuccess || []
        },
      ...
      // Validate that structure has at least one section
      if (ritual.structure.sections.length === 0) {
        ritual.structure.sections = [
          {
            name: 'Opening',
            duration: 5,
          ...
          },
          {
            ...
            duration: Math.max(20, ritual.duration - 10),
      ...
      console.log('✅ Created ritual:', ritual)
      onAddRitual(ritual)
      setRitualInputs({ name: '', type: 'custom', frequency: 'weekly', teamSize: '', teamStage: '', needs: '' })
      setShowForm(false)
    } catch (error) {
      console.error('❌ Failed to generate ritual:', error)
      const errorMessage = error instanceof Error ? error.message : 'Failed to generate ritual. Please try again.'
      alert(errorMessage)
    } finally {
