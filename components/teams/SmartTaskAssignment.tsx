...
import type { SmartTaskAssignment, TeamMember, Task } from '@/lib/teams-types'
...
export default function SmartTaskAssignment({ assignments, members, onAddAssignment, onUpdateMembers, onDeleteAssignment }: SmartTaskAssignmentProps) {
  ...
  const [estimatedHoursInput, setEstimatedHoursInput] = useState<string>('')
  ...
  const handleAddTask = () => {
    ...
    setTasks([...tasks, {
      ...newTask,
      estimatedHours: parseInt(estimatedHoursInput) || 0,
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          if (value === '' || /^\d+$/.test(value)) {
                            setEstimatedHoursInput(value)
                            setNewTask({ ...newTask, estimatedHours: value === '' ? 0 : parseInt(value) || 0 })
                          }
                        }}
                        onFocus={(e) => {
                          // If value is "0", select all so user can type over it
                          if (e.target.value === '0') {
                            e.target.select()
                          }
                        }}
                        onKeyDown={(e) => {
                          // If user starts typing a digit and value is "0", replace it
                          const target = e.currentTarget as HTMLInputElement
                          if (target.value === '0' && /^\d$/.test(e.key) && !e.ctrlKey && !e.metaKey && !e.altKey) {
                            e.preventDefault()
                            setEstimatedHoursInput(e.key)
                            setNewTask({ ...newTask, estimatedHours: parseInt(e.key) || 0 })
                          }
                        }}
                        disabled={isAssigning}
                      />
