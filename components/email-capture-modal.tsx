...
export function EmailCaptureModal() {
  ...
  useEffect(() => {
    ...
    const hasEmail = 
      !!user?.primaryEmailAddress?.emailAddress || // User is authenticated
    ...
    // Use session context as source of truth (Clerk provides auth; session holds profile state)
  ...
  }, [sessionContext?.isSessionActive, sessionContext?.sessionData?.email, sessionContext?.sessionData?.entrepreneurProfile?.onboardingCompleted, user?.primaryEmailAddress?.emailAddress])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (localEmail.trim()) {
      ...
      try {
        ...
        if (user?.primaryEmailAddress?.emailAddress?.toLowerCase() === normalizedEmail) {
          try {
