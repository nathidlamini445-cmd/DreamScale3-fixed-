...
export default function RevenueIntelligence(props: RevenueIntelligenceProps = {}) {
  ...
  // Load analyses from Supabase (if authenticated) or localStorage
  useEffect(() => {
    ...
    const loadAnalyses = async () => {
      ...
      if (user?.id) {
        try {
          ...
          if (sessionData?.revenueAnalyses) {
            setAnalyses(sessionData.revenueAnalyses)
            hasLoadedRef.current = true
            console.log('✅ Loaded revenue analyses from Supabase')
            return
          }
        } catch (error) {
      ...
      try {
        ...
        if (saved) {
          ...
          setAnalyses(parsed)
          hasLoadedRef.current = true
          console.log('✅ Loaded revenue analyses from localStorage')
          return
        }
      } catch (e) {
  ...
  // Save analyses to Supabase (if authenticated) or localStorage
  useEffect(() => {
    if (!hasLoadedRef.current) return
    ...
    const saveAnalyses = async () => {
      try {
        ...
        if (user?.id) {
          try {
            ...
            await supabaseData.saveSessionData(user.id, {
              ...sessionData,
              revenueAnalyses: analyses
            })
            console.log('✅ Saved revenue analyses to Supabase')
            return
          } catch (error) {
        ...
        if (!user?.id && typeof window !== 'undefined') {
          ...
          console.log('✅ Saved revenue analyses to localStorage (unauthenticated)')
        }
      } catch (e) {
        console.error('❌ Failed to save revenue analyses:', e)
      }
    }
    ...
    try {
      ...
      if (user?.id) {
        try {
          ...
          await supabaseData.saveSessionData(user.id, {
            ...sessionData,
            revenueAnalyses: updatedAnalyses
          })
          console.log('✅ Saved new revenue analysis to Supabase immediately')
        } catch (error) {
          console.error('Error saving to Supabase:', error)
        }
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log('✅ Deleted revenue analysis - saved to Supabase')
      } catch (error) {
    ...
    // Only save to localStorage for unauthenticated users
    if (!user?.id && typeof window !== 'undefined') {
      localStorage.setItem('revenue:analyses', JSON.stringify(updatedAnalyses))
    }
