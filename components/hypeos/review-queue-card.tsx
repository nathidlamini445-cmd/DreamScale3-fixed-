            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-blue-700 dark:text-blue-300">
                  Get personalized strategies from{' '}
                  <Link 
                    href={`/bizora?task=${encodeURIComponent(`Review ${queue.totalItems} items in my review queue`)}&instructions=${encodeURIComponent(JSON.stringify([
                      `You have ${queue.totalItems} items to review`,
                      queue.overdueCount > 0 ? `${queue.overdueCount} overdue items need immediate attention` : '',
                      queue.weakenedCount > 0 ? `${queue.weakenedCount} weakened skills need practice` : '',
                      queue.newCount > 0 ? `${queue.newCount} new items to learn` : '',
                      `Estimated time: ${Math.round(queue.estimatedTime / 60 * 10) / 10} minutes`,
                      `Review types: ${queue.items.map(i => getReviewTypeLabel(i.reviewType)).filter((v, i, a) => a.indexOf(v) === i).join(', ')}`
                    ].filter(Boolean)))}`}
                    className="font-semibold underline hover:text-blue-900 dark:hover:text-blue-100"
                  >
                    Bizora AI
                  </Link>
                  {' '}to efficiently manage your review queue!
                </p>
              </div>
            </div>
          </div>
