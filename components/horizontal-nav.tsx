...
export function HorizontalNav() {
  ...
  const shouldShowNotification = (label: string) => {
    switch (label) {
      ...
      case "PitchPoint": return false // Notifications disabled - users create content on their own
              ...
              <div key={index} className="flex items-center gap-1">
                <Link
                  ...
                  onClick={(e) => {
                    ...
                    // Show loading overlay for Bizora AI
                    if (item.label === "Bizora AI" && pathname !== "/bizora") {
                      setOpeningBizora(true)
                      // Navigate immediately - overlay will hide when page loads
                ...
                >
                  ...
                  <span className={cn(
                    ...
                    item.label === "Bizora AI" && !isActive ? "bizora-ai-text" : "",
                    isActive ? (item.label === "Studio" ? "text-black font-bold" : "text-white") : item.label === "Bizora AI" ? "" : "text-gray-700 dark:text-white group-hover:text-gray-800 dark:group-hover:text-white"
                  )}>{item.label}</span>
