...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    const checkAuthAndOnboarding = async () => {
      ...
      const sessionResult = await withTimeout(supabase.auth.getSession(), SESSION_TIMEOUT_MS, () => null)

      if (cancelled) return

      if (sessionResult === null) {
        console.warn('⚠️ [LANDING] getSession timed out — showing sign-in entry points')
      ...
      if (sessionError) {
        ...
        setError('Could not verify saved session. You can still sign in below.')
        setIsAuthenticated(false)
        setOnboardingCompleted(false)
        setSessionProbeDone(true)
        return
