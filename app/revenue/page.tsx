...
export default function RevenuePage() {
  ...
  useEffect(() => {
    const loadSystems = async () => {
      // Try Supabase first if authenticated (for cross-device sync)
      if (user?.id) {
        try {
          const systemsData = await supabaseData.loadSystemsData(user.id)
          if (systemsData?.systems) {
            setSystems(systemsData.systems)
            setTotalSystems(systemsData.systems.length)
            return
          }
        } catch (error) {
          console.error('Error loading systems from Supabase:', error)
        }
      }
    ...
    loadSystems()
  }, [sessionContext?.sessionData?.systems?.systems, user?.id])
