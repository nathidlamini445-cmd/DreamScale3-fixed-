...
function SuggestionsPage() {
  ...
  useEffect(() => {
    const loadSuggestions = () => {
      ...
      // If old cache was detected or no cache but we have onboarding data, generate new recommendations
      // IMPORTANT: Clear old suggestions immediately so generic ones don't show
      if (isOldCache) {
        setMainSuggestions([])
        setDailySuggestions([])
      }
      
      // Check if generate=true is in URL (from "Get More Detailed Steps" button)
      const shouldGenerate = searchParams.get('generate') === 'true'
      
      // Show generic recommendations immediately as fallback
      if ((shouldGenerate || isOldCache || !cachedMain) && data) {
        // If generate=true, clear cache to force fresh generation
        if (shouldGenerate) {
          console.log('🔄 Force generating recommendations from button click...')
          localStorage.removeItem('mainRecommendations')
          localStorage.removeItem('dailyRecommendations')
        }
        ...
        // Then try to generate AI recommendations in background (optional)
        // setTimeout(() => {
