...
export default function BizoraAIPage() {
  ...
  // Auto-scroll to bottom when messages change or when thinking starts
  useEffect(() => {
...
Please provide personalized guidance on how to complete this task. Give me detailed, actionable steps tailored to my situation.`
    ...
    try {
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          favoriteSong: entrepreneurProfile?.favoriteSong || undefined
        }),
      ...
      // Show the actual error message if available, otherwise use fallback
      if (error instanceof Error && error.message && error.message !== 'API request failed' && !error.message.includes('API request failed with status')) {
        return `Error: ${error.message}. Please check your API configuration or try again.`
      }
      // Fallback to simple response only if we have no error information
    ...
    if (files) {
      ...
      for (const file of Array.from(files)) {
        ...
        if (file.type === 'application/pdf') {
          alert('PDF upload is currently disabled. Please use images or text files instead.')
          continue
        }
        ...
        try {
          ...
          if (response.ok) {
            ...
            newAttachments.push({
              ...
              processed: true,
              isImage: result.file.isImage || file.type.startsWith('image/'),
              isPdf: result.file.isPdf || file.type === 'application/pdf',
              imageData: result.file.imageData
            })
          } else {
  ...
  return (
    <div className={`flex flex-col min-h-screen bg-white dark:bg-slate-950 ${inter.variable}`} style={{ fontFamily: 'var(--font-inter), -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
              ...
              {!isSidebarCollapsed && (
                <div className="flex items-center gap-2">
                  <Link
                    ...
                    title="Back to Home"
                  >
                    <Home className="h-5 w-5 text-gray-500 dark:text-gray-400 group-hover:text-[#39d2c0] transition-colors" />
                  </Link>
                  <button
                    onClick={() => {
                      setMessages([])
            ...
            <div className="flex gap-2 items-end">
              <div className="flex-1 relative">
                <textarea
                  ...
                  className={`w-full min-h-[48px] max-h-96 resize-none rounded-lg pr-24 border dark:bg-gray-800 dark:text-gray-100 focus:border-[#39d2c0] focus:ring-2 focus:ring-[#39d2c0]/20 focus:shadow-sm focus:shadow-[#39d2c0]/10 p-2.5 text-sm transition-all duration-200 overflow-y-auto ${
                    showGlowEffect 
