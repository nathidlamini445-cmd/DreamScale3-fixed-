...
/**
 * Picks the best supported MediaRecorder mime type for browser audio capture.
 * Returns "" when MediaRecorder cannot pick a default; the caller should fall back
 * to letting the browser choose (passing no mimeType in the options).
 */
...
/**
 * Returns the browser's SpeechRecognition constructor (Chrome/Edge/Safari support
 * `webkitSpeechRecognition`; standards-track `SpeechRecognition` is sparsely shipped
 * but supported on some Chromium builds). Returns null on Firefox and any non-browser
 * environment, in which case the caller should fall back to MediaRecorder + server
 * transcription.
 */
...
interface Attachment {
  ...
  name: string
  type: 'file' | 'link'
  /** MIME type for file attachments (e.g. "application/pdf"). */
  mimeType?: string
  url?: string
  file?: File
  content?: string
...
export default function BizoraAIPage() {
  ...
  /** True once the MediaRecorder has actually started capturing in the panel. */
  const [isRecording, setIsRecording] = useState(false)
  const mediaRecorderRef = useRef<MediaRecorder | null>(null)
  const recordedChunksRef = useRef<Blob[]>([])
  /** Live Web Speech API recognizer (Chrome/Edge/Safari). Null on browsers without support
   *  or while the recorder fallback path is being used. */
  const speechRecognitionRef = useRef<any>(null)
  /** Accumulated FINAL transcript text for the current voice session. Interim words are
   *  rendered live but never written here; only finalized segments get appended so that
   *  re-renders during the same utterance are stable. */
  const finalTranscriptRef = useRef("")
  /** True while a Web Speech API session is the active engine. False when we fell back
   *  to MediaRecorder. Lets stopVoiceInput pick the right teardown path. */
  const usingSpeechRecognitionRef = useRef(false)
  /** Text already in the textarea when recording started; we append the transcript to this. */
  ...
  /** Stops mic + recorder + waveform only. Does not hide the panel. */
  ...
  const getMicErrorHint = useCallback((e: unknown): string => {
    ...
    if (
      ...
      return `Microphone permission is blocked for this site${tag}. In Chrome: click the small icon left of the URL → Site settings → Microphone → Allow. Then reload this page (Ctrl+Shift+R).`
    }
  ...
  /** Retry button under the warning: fully tear down and restart the recorder flow. */
  ...
  /**
   * After recording stops we send the audio to /api/transcribe (server-side Gemini)
   * and append the result to whatever text was already in the textarea.
   */
  const finalizeRecordingAndTranscribe = useCallback(async (chunks: Blob[], mime: string) => {
    ...
    try {
      ...
      if (blob.size < 1024) {
        // Less than ~1KB likely means user clicked stop instantly with no real audio.
        setIsTranscribing(false)
        closeVoicePanelForUser()
        return
      }
      ...
      if (transcript) {
        ...
        const next = `${prefix}${sep}${transcript}`
        ...
        setMicPermissionHint("We didn’t hear any speech in that recording. Try again — speak a bit louder or closer to the mic.")
      }
      setIsTranscribing(false)
      if (transcript) {
        closeVoicePanelForUser()
      ...
      console.error("transcribe POST failed:", e)
  ...
  const stopVoiceInput = useCallback(() => {
    // Web Speech API path: the live transcript is already in the textarea; just stop
    // the recognizer and close the panel. No upload or post-processing needed.
  ...
  const startVoiceInput = useCallback(async () => {
    if (isThinking || isVoicePanelOpen || isTranscribing) return
    ...
    // Diagnostic-only: log what the Permissions API thinks, but DO NOT short-circuit
    // on its result. On production deploys (custom domains + Permissions-Policy headers
    // + occasionally stale Chrome state), this API has been observed to return "denied"
    // even when the user has explicitly granted permission in Site Settings. The only
    // reliable source of truth is getUserMedia itself — if permission is genuinely
    // blocked, it throws NotAllowedError and our existing handler shows the right
    // friendly message anyway. So we always attempt the real call.
    let permsApiState = "unknown"
    try {
      ...
      if (perms?.query) {
        ...
        permsApiState = status.state
        console.log("🎤 [VOICE] Permissions API state:", status.state, "(advisory only)")
      }
    } catch { /* Permissions API may not exist; ignore and proceed to getUserMedia */ }

    console.log("🎤 [VOICE] Pre-flight diagnostics:", {
      ...
      permissionsApiState: permsApiState,
    })

    let stream: MediaStream
    try {
      stream = await navigator.mediaDevices.getUserMedia({ audio: true })
      console.log("🎤 [VOICE] getUserMedia success — tracks:", stream.getAudioTracks().map(t => ({ label: t.label, enabled: t.enabled, muted: t.muted, readyState: t.readyState })))
    } catch (e) {
      const errName = e instanceof DOMException ? e.name : (e as { name?: string })?.name ?? "unknown"
      const errMsg = e instanceof Error ? e.message : String(e)
      console.error("🎤 [VOICE] getUserMedia FAILED:", { name: errName, message: errMsg, error: e })
      console.error("🎤 [VOICE] Diagnostic context:", {
        isSecureContext: typeof window !== "undefined" ? window.isSecureContext : "n/a",
        protocol: typeof window !== "undefined" ? window.location.protocol : "n/a",
        permissionsApiState: permsApiState,
      })
    ...
    try {
      ...
      await ctx.resume()
      startRealWaveformLoop(analyser)
    } catch (e) {
      console.warn("Audio waveform setup failed (recording will still work):", e)
      analyserRef.current = null
      if (audioContextRef.current) {
    ...
    // Prefer the browser-native Web Speech API for live, word-by-word transcription.
    // It runs entirely in the browser, has no API cost, and (unlike the Gemini batch
    // path) produces interim results we can render as the user speaks. We fall back
    // to MediaRecorder + /api/transcribe only if the browser lacks SpeechRecognition
    // (notably Firefox).
    ...
    if (SR) {
      ...
      try {
        ...
        console.warn("SpeechRecognition constructor failed; falling back to recorder:", e)
        recognition = null
      }
      if (recognition) {
        recognition.continuous = true
        recognition.interimResults = true
        // Use the user's UI language as a hint; SpeechRecognition silently accepts
        // anything BCP-47-ish and falls back to the engine default if unsupported.
        ...
        recognition.onresult = (event: any) => {
          ...
          const live = `${prefix}${sep}${finalTranscriptRef.current}${interim}`
          setCurrentMessage(live)
        ...
        recognition.onerror = (ev: any) => {
          // "no-speech" and "aborted" are benign — the engine just stopped hearing
          // sound or the user pressed stop. Anything else is a real failure.
          const err: string = ev?.error || "unknown"
          if (err === "no-speech" || err === "aborted") return
          console.warn("SpeechRecognition error:", err)
          if (err === "not-allowed" || err === "service-not-allowed") {
            setMicPermissionHint(
              "Microphone permission is blocked for this site [not-allowed]. In Chrome: click the small icon left of the URL → Site settings → Microphone → Allow. Then reload this page (Ctrl+Shift+R)."
            )
          } else if (err === "audio-capture") {
            setMicPermissionHint("No microphone is available to this page. Plug one in or pick a working device in Windows sound settings.")
          } else if (err === "network") {
            setMicPermissionHint("Live transcription needs an internet connection to Google's speech service. Check your network and try again.")
          } else {
            setMicPermissionHint(`Voice recognition failed: ${err}`)
          }
        }

        recognition.onend = () => {
          // Fired both on user-initiated stop and when the engine ends a session
          // (e.g. long silence). The textarea already holds the final transcript,
          // so we just settle UI state. closeVoicePanelForUser handles the rest.
          setIsRecording(false)
          if (usingSpeechRecognitionRef.current) {
            usingSpeechRecognitionRef.current = false
            speechRecognitionRef.current = null
        ...
        try {
          ...
          console.warn("SpeechRecognition.start failed; falling back to recorder:", e)
          speechRecognitionRef.current = null
          usingSpeechRecognitionRef.current = false
        }
      }
    ...
    // Fallback: MediaRecorder + server-side Gemini transcription (single-shot, on stop).
    const mime = pickRecorderMime()
    let recorder: MediaRecorder
    try {
      recorder = mime ? new MediaRecorder(stream, { mimeType: mime }) : new MediaRecorder(stream)
    } catch (e) {
      console.error("MediaRecorder constructor failed:", e)
      setMicPermissionHint(
        e instanceof Error
          ? `Recorder failed to start: ${e.message}`
          : "Recorder failed to start in this browser."
      )
      teardownVoiceEngineOnly()
      return
    }
    ...
    recorder.onerror = (ev) => {
      ...
      setMicPermissionHint("Recording failed unexpectedly. Please try again.")
    }
    recorder.onstop = () => {
      ...
      analyserRef.current = null
      mediaRecorderRef.current = null
      void finalizeRecordingAndTranscribe(chunks, usedMime)
    }
    ...
    try {
      ...
      setMicPermissionHint(
        e instanceof Error
          ? `Recorder could not start: ${e.message}`
          : "Recorder could not start. Try reloading the page."
      )
      teardownVoiceEngineOnly()
      setIsRecording(false)
    }
    ...
    applyRecordingHeartbeat,
    cancelVoiceRaf,
    finalizeRecordingAndTranscribe,
    getMicErrorHint,
  ...
  useEffect(() => {
    ...
    const taskTitle = searchParams?.get('task') ?? null
    const instructionsParam = searchParams?.get('instructions') ?? null
...
Please provide personalized guidance on how to complete this task. Give me detailed, actionable steps tailored to my situation.`
  ...
  useEffect(() => {
    ...
    // Re-run when the email becomes available AND when Supabase conversations arrive
    // asynchronously through the session context. Without the second dep, the effect
    // fires once before chat data has loaded, sees nothing, and never re-runs — which
    // is why old chats only appeared after a manual reload.
  }, [sessionContext?.sessionData?.email, sessionContext?.sessionData?.chat?.conversations?.length])
  ...
  const handleAIResponse = async (userMessage: string): Promise<string> => {
    try {
      ...
      // NOTE: `att.type` is the attachment kind ('file' | 'link'); the file's
      // MIME type lives on `att.mimeType` (or `att.file?.type`).
      const mimeOf = (att: Attachment) => att.mimeType ?? att.file?.type ?? ''
      const fileContent = attachments
        .filter(att =>
          ...
          !att.isImage &&
          !att.isPdf &&
          !mimeOf(att).startsWith('image/') &&
          mimeOf(att) !== 'application/pdf'
        )
      ...
      const fileAttachments = attachments
        .filter(att =>
          att.type === 'file' &&
          (att.isImage || att.isPdf || mimeOf(att).startsWith('image/') || mimeOf(att) === 'application/pdf')
        )
        .map(att => ({
          name: att.name,
          type: mimeOf(att),
          isImage: att.isImage || mimeOf(att).startsWith('image/'),
          isPdf: att.isPdf || mimeOf(att) === 'application/pdf',
          imageData: att.imageData || att.content,
          content: att.content,
        }))
  ...
  const handleSendMessage = () => {
    ...
    responseTimeoutRef.current = setTimeout(async () => {
      // `aiResponse` is undefined if the request was aborted by the user, in
      // which case we should NOT save anything to history.
      ...
      // Update conversation history — but only if we actually produced an
      // AI response. (Aborted requests leave `aiResponse` undefined.)
      ...
      if (messages.length === 0) {
        ...
        const newConversation = {
          ...
          lastMessage: finalAiResponse.content.substring(0, 50) + "...",
          timestamp: new Date(),
          messages: [userMessage, finalAiResponse]
        }
        setConversationHistory(prev => {
          ...
          const updated = prev.map(conv =>
            conv.id === selectedConversationId
              ? {
                  ...
                  lastMessage: finalAiResponse.content.substring(0, 50) + "...",
                  timestamp: new Date(),
                  messages: [...conv.messages, userMessage, finalAiResponse]
                }
              : conv
          )
                ...
                <div className="flex items-center gap-1 sm:gap-3">
                  <div className="flex min-w-0 flex-1 flex-col gap-1.5 sm:flex-row sm:items-center sm:gap-3">
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      <VoiceWaveform className="h-6 shrink-0" levels={waveLevels} active={isRecording || isTranscribing} />
                    </div>
                  ...
                  <button
                    ...
                    disabled={isTranscribing || (!isRecording && !!micPermissionHint)}
                    ...
                    title={isTranscribing ? "Transcribing audio…" : isRecording ? "Stop recording and transcribe" : "Close"}
                    aria-label={isTranscribing ? "Transcribing audio" : isRecording ? "Stop recording" : "Close voice panel"}
                  >
