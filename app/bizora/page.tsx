...
export default function BizoraAIPage() {
  ...
  const startVoiceInput = useCallback(async () => {
    ...
    // Pre-flight: if the browser already knows permission is denied, short-circuit
    // with a precise message instead of showing "Recording…" while getUserMedia rejects.
    try {
      ...
      if (perms?.query) {
        ...
        if (status.state === "denied") {
          setMicPermissionHint(
            "Microphone permission is blocked for this site [denied]. In Chrome: click the small icon left of the URL → Site settings → Microphone → Allow. Then reload this page (Ctrl+Shift+R)."
          )
          startFakeWaveformLoop()
          return
        }
      }
    } catch { /* permissions API may not exist; ignore and try getUserMedia */ }
