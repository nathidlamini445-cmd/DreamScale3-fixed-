...
export default function SkillDropsPage() {
  ...
  useEffect(() => {
    ...
    try {
      const raw = localStorage.getItem(`dreamscale:skilldrops:completed:${userEmail}`)
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed)) {
          setCompletedLessons(parsed.filter((n: unknown): n is number => typeof n === 'number'))
        }
      }
    } catch (err) {
              ...
              <div className="flex items-center justify-between py-1">
                <div>
                  <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
                    Skill<span className="text-[#39d2c0]">Drops</span>
                  </h1>
        ...
        <LessonDetailModal
          ...
          onComplete={() => {
            if (!completedLessons.includes(selectedLesson.id)) {
              const next = [...completedLessons, selectedLesson.id]
              setCompletedLessons(next)
              if (typeof window !== 'undefined' && userEmail) {
                try {
                  localStorage.setItem(
                    `dreamscale:skilldrops:completed:${userEmail}`,
                    JSON.stringify(next),
                  )
                } catch (err) {
