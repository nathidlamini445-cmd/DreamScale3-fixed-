...
export function Header({ onSettingsClick }: HeaderProps = {}) {
  ...
  // Display identity (omit raw email until we derive a friendly token)
  const resolvedName =
    sessionContext?.sessionData?.entrepreneurProfile?.name?.trim() ||
    profile?.user_name?.trim() ||
    profile?.full_name?.trim() ||
    (user?.user_metadata as Record<string, string | undefined> | undefined)?.given_name?.trim() ||
    user?.user_metadata?.full_name?.trim() ||
    user?.user_metadata?.name?.trim() ||
    null

  const userInitialSource = resolvedName || user?.email || null

  const userFirstName = useMemo(() => {
    if (resolvedName) {
      const first = resolvedName.split(/\s+/)[0]
      return first || null
    }
    return firstAlphabeticTokenFromEmail(user?.email)
  ...
  useEffect(() => {
    if (userInitialSource) {
      console.log('✅ Header: Resolved display:', userInitialSource, {
        ...
        fromProfileUserName: !!(profile as { user_name?: string | null } | null)?.user_name,
        fromProfileFullName: !!profile?.full_name,
        fromUserMetadata:
          !!user?.user_metadata?.full_name ||
          !!user?.user_metadata?.name ||
          !!(user?.user_metadata as Record<string, string | undefined> | undefined)?.given_name,
      })
    } else if (user) {
      console.warn('⚠️ Header: User logged in but no name resolved', {
        ...
        profileUserName: (profile as { user_name?: string | null } | null)?.user_name,
        profileName: profile?.full_name,
        userMetadataName: user?.user_metadata?.full_name || user?.user_metadata?.name,
        givenName: (user?.user_metadata as Record<string, string | undefined> | undefined)?.given_name,
    ...
    userInitialSource,
    profile,
    sessionContext?.sessionData?.entrepreneurProfile?.name,
    user?.user_metadata,
    user?.email,
  ])
  ...
  const resolveGreetingFirstName = (): string =>
    userFirstName || friendlyFallbackFirst()
