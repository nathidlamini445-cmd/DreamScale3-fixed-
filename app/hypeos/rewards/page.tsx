...
export default function RewardsPage() {
  ...
  useEffect(() => {
    // Load user data - check session context first (for authenticated users), then localStorage
    const loadUserData = () => {
      try {
        ...
        // For authenticated users, check session context first (Supabase data)
        if (authUser?.id && sessionContext?.sessionData?.hypeos) {
          const hypeosData = sessionContext.sessionData.hypeos;
          sessionUser = hypeosData.user;
          goalsList = hypeosData.allGoals || [];
          redeemed = hypeosData.redeemedRewards || [];
          console.log('✅ Rewards: Loaded from session context (authenticated user)');
        }
        
        // Also check localStorage (for unauthenticated users OR as fallback)
        if (!sessionUser) {
          const stored = localStorage.getItem('dreamscale_session');
          if (stored) {
            const parsed = JSON.parse(stored);
            sessionUser = parsed?.hypeos?.user;
            goalsList = parsed?.hypeos?.allGoals || [];
            redeemed = parsed?.hypeos?.redeemedRewards || [];
            console.log('✅ Rewards: Loaded from localStorage');
          }
        }
  ...
  return (
    <div className="h-screen bg-white dark:bg-slate-950 overflow-y-auto overflow-x-hidden">
      <RewardStore 
