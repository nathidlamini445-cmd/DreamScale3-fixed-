...
import ProgressCard from '@/components/hypeos/progress-card';
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950">
      ...
      <div className="fixed top-4 left-4 z-50">
        <Link
          href="/"
          className="flex items-center gap-2 px-3 py-2 rounded-md bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors duration-200 shadow-sm"
        >
      ...
      <div className="container mx-auto px-6 py-12 pt-16 max-w-3xl">
        {/* Header */}
...
export default function HypeOSPage() {
  ...
  // CRITICAL: Don't change this during load to prevent navigation issues
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        if (stored) {
          ...
          // Only consider it a goal if it has a valid goalTitle (not empty)
          ...
          const hasValidGoal = (goalsList.length > 0 && goalsList[0]?.goalTitle && goalsList[0].goalTitle.trim() !== '') ||
                              (parsed?.hypeos?.user?.goalTitle && parsed?.hypeos?.user?.goalTitle.trim() !== '');
          // If we have valid goal, don't show onboarding (prevents navigation)
          return !hasValidGoal; // Show onboarding only if no valid goals exist
        }
      } catch (e) {
        // Ignore errors
      }
    }
    // Default to false if we're on /hypeos page - let data loading determine if onboarding is needed
  ...
  // Get auth status - skip localStorage for authenticated users
  const { user: authUser } = useAuth()
  
  // Enable scrolling on HypeOS page - MUST be before any conditional returns
  useEffect(() => {
  ...
  // Load HypeOS data - try Supabase first for authenticated users, then localStorage
  useEffect(() => {
    if (hasLoadedFromSession.current) return; // Already loaded
    
    // For authenticated users, load directly from Supabase if session context doesn't have it yet
    if (authUser?.id) {
      // Check if session context already has the data
      if (sessionData?.hypeos?.allGoals && sessionData.hypeos.allGoals.length > 0) {
        console.log('✅ Session context has HypeOS data, will load in session useEffect')
        return // Let the session useEffect handle it
      }
      
      // Session context doesn't have data yet - load directly from Supabase
      const loadFromSupabase = async () => {
        try {
          const hypeosData = await supabaseData.loadHypeOSData(authUser.id)
          if (hypeosData) {
            console.log('✅ Loaded HypeOS data directly from Supabase:', {
              allGoalsCount: hypeosData.allGoals?.length || 0,
              questsCount: hypeosData.quests?.length || 0,
              tasksCount: hypeosData.tasks?.length || 0
            })
            
            // Set the data immediately
            if (hypeosData.allGoals && hypeosData.allGoals.length > 0) {
              setAllGoals(hypeosData.allGoals)
              const firstGoal = hypeosData.allGoals[0]
              if (firstGoal && !currentGoalId) {
                setCurrentGoalId(firstGoal.goalTitle + '-' + firstGoal.category)
              }
            }
            
            if (hypeosData.quests && hypeosData.quests.length > 0) {
              setQuests(hypeosData.quests)
            }
            
            if (hypeosData.user) {
              setUser(hypeosData.user)
            }
            
            if (hypeosData.tasks && hypeosData.tasks.length > 0) {
              const today = new Date().toDateString()
              if (hypeosData.tasksLastDate === today) {
                const enrichedTasks = enrichTasksWithInstructions(hypeosData.tasks)
                setTasks(enrichedTasks)
              }
            }
            
            if (hypeosData.miniWins && hypeosData.miniWins.length > 0) {
              setMiniWins(hypeosData.miniWins)
            }
            
            // Update session context with loaded data so it's available everywhere
            updateHypeOSData({
              allGoals: hypeosData.allGoals || [],
              quests: hypeosData.quests || [],
              user: hypeosData.user,
              tasks: hypeosData.tasks || [],
              miniWins: hypeosData.miniWins || [],
              tasksLastDate: hypeosData.tasksLastDate
            })
            
            setIsLoading(false)
            // Don't set hasLoadedFromSession here - let session useEffect also run to ensure consistency
          } else {
            console.log('📭 No HypeOS data found in Supabase yet')
            setIsLoading(false)
          }
        } catch (error) {
          console.error('Error loading HypeOS data from Supabase:', error)
          setIsLoading(false)
        }
      }
    ...
    try {
      if (typeof window !== 'undefined') {
        ...
        // Load tasks from localStorage FIRST (ABSOLUTE PRIORITY - like Bizora)
        // CRITICAL: localStorage is the source of truth for completed tasks - NEVER overwrite
        const savedTasks = localStorage.getItem('hypeos:tasks');
        const savedUserData = localStorage.getItem('hypeos:user');
        const savedTasksDate = localStorage.getItem('hypeos:tasksLastDate');
        
        if (savedTasks && savedTasksDate === today) {
          // We have saved tasks for today - ALWAYS use them! (Absolute priority)
          try {
            const parsed = JSON.parse(savedTasks);
            const completedCount = parsed.filter((t: any) => t.completed).length;
            console.log('✅ Loaded tasks from localStorage (ABSOLUTE PRIORITY):', parsed.length, 'tasks', {
            ...
            // Also load user data to get updated points
            if (savedUserData) {
              try {
                const parsedUser = JSON.parse(savedUserData);
                console.log('✅ Loaded user data from localStorage (ABSOLUTE PRIORITY):', {
                  points: parsedUser.hypePoints,
                  streak: parsedUser.currentStreak
                });
                setUser(parsedUser);
              } catch (e) {
            ...
            // Mark as loaded to prevent session from overwriting - CRITICAL
            hasLoadedFromSession.current = true;
            console.log('🔒 localStorage data loaded - session data will be skipped to preserve completed tasks');
          } catch (e) {
        ...
        if (savedQuests) {
          ...
          // Initialize quests if they don't exist yet
          const initialQuests = initializeQuests();
          setQuests(initialQuests);
          console.log('✅ Initialized quests:', initialQuests.length, 'quests');
        }
      ...
      console.warn('Failed to load Venture Quest data from localStorage', e);
      // Still set loading to false even on error
      setIsLoading(false);
    }
  }, []); // Only run once on mount - BEFORE session loading
  
  // Save tasks directly to localStorage ONLY for unauthenticated users
  // Authenticated users' data is saved to Supabase via session context
  useEffect(() => {
    if (authUser?.id) return // Skip localStorage for authenticated users
    
    try {
      if (typeof window !== 'undefined' && tasks.length > 0) {
        localStorage.setItem('hypeos:tasks', JSON.stringify(tasks));
        console.log('💾 Saved tasks to localStorage (unauthenticated):', tasks.length, 'tasks');
      }
    } catch (e) {
  ...
  // Save miniWins directly to localStorage ONLY for unauthenticated users
  useEffect(() => {
    if (authUser?.id) return // Skip localStorage for authenticated users
    ...
    try {
      if (typeof window !== 'undefined' && miniWins.length > 0) {
        ...
        console.log('💾 Saved miniWins to localStorage (unauthenticated):', miniWins.length, 'miniWins');
      }
    } catch (e) {
      console.warn('Failed to save miniWins to localStorage', e);
    }
  }, [miniWins, authUser?.id]);
  
  // Save user data directly to localStorage ONLY for unauthenticated users
  useEffect(() => {
    if (authUser?.id) return // Skip localStorage for authenticated users
    ...
    try {
      if (typeof window !== 'undefined' && user) {
        ...
        console.log('💾 Saved user to localStorage (unauthenticated):', user.goalTitle, 'Points:', user.hypePoints);
      }
    } catch (e) {
  ...
  // Save allGoals directly to localStorage ONLY for unauthenticated users
  useEffect(() => {
    if (authUser?.id) return // Skip localStorage for authenticated users
    ...
    try {
      if (typeof window !== 'undefined' && allGoals.length > 0) {
        ...
        console.log('💾 Saved goals to localStorage (unauthenticated):', allGoals.length, 'goals');
      }
    } catch (e) {
      console.warn('Failed to save goals to localStorage', e);
    }
  }, [allGoals, authUser?.id]);
  
  // Save quests directly to localStorage ONLY for unauthenticated users
  useEffect(() => {
    if (authUser?.id) return // Skip localStorage for authenticated users
    ...
    try {
      if (typeof window !== 'undefined' && quests.length > 0) {
        ...
        console.log('💾 Saved quests to localStorage (unauthenticated):', quests.length, 'quests');
      }
    } catch (e) {
  ...
  // Tries AI first, falls back to generic tasks if AI fails
  const generateInitialTasks = async (userData: User) => {
    // Safety check: ensure we have valid user data
    if (!userData || !userData.goalTitle || userData.goalTitle.trim() === '') {
      console.warn('⚠️ generateInitialTasks: Invalid userData, missing goalTitle', userData);
      return; // Don't generate tasks without a valid goal
    }

    // Try AI generation first (with timeout and fallback)
    const tryAITaskGeneration = async (): Promise<Task[] | null> => {
      try {
        // Extract onboarding data from session
        const entrepreneurProfile = sessionData?.entrepreneurProfile;
        
        // Log what we have for debugging
        console.log('🔍 Checking for onboarding data:', {
          hasSessionData: !!sessionData,
          hasEntrepreneurProfile: !!entrepreneurProfile,
          goalTitle: userData.goalTitle,
          category: userData.category
        });
        
        // If no entrepreneur profile, try to use goal data as fallback
        if (!entrepreneurProfile) {
          console.log('📝 No entrepreneur profile found, but will try AI with goal data');
          // Continue with minimal data - AI can still generate tasks
        }

        // Prepare onboarding data for AI (use entrepreneurProfile if available, otherwise use goal data)
        ...
        console.log('📤 Sending onboarding data to AI:', onboardingData);

        console.log('🤖 Attempting AI task generation...');
        ...
        const apiPromise = fetch('/api/tasks/generate', {
          method: 'POST',
        ...
        // Convert AI tasks to our format (use daily tasks, select 4)
        ...
        const selectedTasks = dailyTasks.slice(0, 4).map((task: any, index: number) => ({
          ...
          estimatedTime: '45 min', // Default estimate
          howToComplete: undefined // AI tasks don't have pre-built instructions
        }));
        ...
        console.warn('⚠️ AI task generation failed, falling back to generic tasks:', error.message);
        return null; // Return null to trigger fallback
      }
    };

    // Helper function for mini wins (needed for both AI and generic paths)
    ...
    // Try AI first, then fallback to generic
    const aiTasks = await tryAITaskGeneration();
    
    // If AI succeeded, use those tasks; otherwise use generic
    if (aiTasks && aiTasks.length > 0) {
      ...
      const initialQuests = initializeQuests();
      setQuests(initialQuests);
      return; // Exit early, AI tasks are set
    }

    // Fallback to generic tasks (original logic)
    console.log('📋 Using generic tasks (AI unavailable or failed)');
    
    const getNicheSpecificTasks = (category: string, goalTitle: string) => {
      // Ensure goalTitle is valid and lowercase for consistent formatting
      const safeGoalTitle = goalTitle && goalTitle.trim() !== '' ? goalTitle.toLowerCase().trim() : 'your goal';
      const baseTasks = {
        revenue: [
          { title: `Research revenue streams for ${safeGoalTitle}`, points: 200, impact: 'high' as const, category: 'research', time: '45 min' },
          ...
          { title: `Identify 3 new revenue opportunities for ${safeGoalTitle}`, points: 180, impact: 'high' as const, category: 'research', time: '40 min' },
        ...
        audience: [
          { title: `Define your target audience for ${safeGoalTitle}`, points: 200, impact: 'high' as const, category: 'research', time: '45 min' },
          ...
          { title: `Build an email list strategy for ${safeGoalTitle}`, points: 220, impact: 'high' as const, category: 'strategy', time: '50 min' },
        ...
        product: [
          { title: `Conduct market research for ${safeGoalTitle}`, points: 200, impact: 'high' as const, category: 'research', time: '45 min' },
          ...
          { title: `Design user experience flow for ${safeGoalTitle}`, points: 220, impact: 'high' as const, category: 'design', time: '50 min' },
        ...
        marketing: [
          { title: `Create marketing strategy for ${safeGoalTitle}`, points: 250, impact: 'high' as const, category: 'strategy', time: '60 min' },
          ...
          { title: `Develop brand messaging for ${safeGoalTitle}`, points: 220, impact: 'high' as const, category: 'strategy', time: '50 min' },
        ...
        content: [
          { title: `Develop content strategy for ${safeGoalTitle}`, points: 200, impact: 'high' as const, category: 'strategy', time: '45 min' },
          ...
          { title: `Write blog post about ${safeGoalTitle}`, points: 220, impact: 'high' as const, category: 'writing', time: '50 min' },
        ...
        business: [
          { title: `Create business growth plan for ${safeGoalTitle}`, points: 250, impact: 'high' as const, category: 'strategy', time: '60 min' },
          ...
          { title: `Develop competitive advantage for ${safeGoalTitle}`, points: 220, impact: 'high' as const, category: 'strategy', time: '50 min' },
        ...
        skills: [
          { title: `Identify key skills needed for ${safeGoalTitle}`, points: 150, impact: 'medium' as const, category: 'research', time: '30 min' },
          ...
          { title: `Find mentors or experts in ${safeGoalTitle}`, points: 120, impact: 'medium' as const, category: 'networking', time: '25 min' },
        ...
        creative: [
          { title: `Brainstorm creative concepts for ${safeGoalTitle}`, points: 200, impact: 'high' as const, category: 'ideation', time: '45 min' },
          ...
          { title: `Experiment with new creative techniques for ${safeGoalTitle}`, points: 220, impact: 'high' as const, category: 'creation', time: '50 min' },
        ...
        learning: [
          { title: `Research best resources for ${safeGoalTitle}`, points: 150, impact: 'medium' as const, category: 'research', time: '30 min' },
          ...
          { title: `Find study groups or learning communities for ${safeGoalTitle}`, points: 120, impact: 'medium' as const, category: 'networking', time: '25 min' },
        ...
        fitness: [
          { title: `Create fitness plan for ${safeGoalTitle}`, points: 200, impact: 'high' as const, category: 'planning', time: '45 min' },
          ...
          { title: `Research workout routines for ${safeGoalTitle}`, points: 120, impact: 'medium' as const, category: 'research', time: '25 min' },
    ...
    // Ensure goalTitle exists before generating tasks
    const safeGoalTitle = userData.goalTitle && userData.goalTitle.trim() !== '' 
      ? userData.goalTitle 
      : null;
    
    if (!safeGoalTitle) {
      console.warn('⚠️ Cannot generate tasks: goalTitle is missing or empty', userData);
      return;
    }
    
    const nicheTasks = getNicheSpecificTasks(userData.category || 'revenue', safeGoalTitle);
  ...
  useEffect(() => {
    // CRITICAL: Check localStorage FIRST before processing session data
    // This prevents session data from overwriting completed tasks
    if (typeof window !== 'undefined') {
      const savedTasks = localStorage.getItem('hypeos:tasks');
      const savedTasksDate = localStorage.getItem('hypeos:tasksLastDate');
      const today = new Date().toDateString();
      ...
      if (savedTasks && savedTasksDate === today) {
        // localStorage has today's tasks - check if they have completed tasks
        try {
          const parsed = JSON.parse(savedTasks);
          const completedCount = parsed.filter((t: any) => t.completed).length;
          
          if (completedCount > 0) {
            // localStorage has completed tasks - mark as loaded to prevent session overwrite
            console.log('🔒 localStorage has completed tasks - session data will be skipped:', {
              completed: completedCount,
              total: parsed.length
            });
            hasLoadedFromSession.current = true;
    ...
    // Only skip if we're currently saving
    ...
    const sessionKey = JSON.stringify({
      ...
      questsCount: (sessionData?.hypeos as any)?.quests?.length || 0,
    ...
    const checkUserStatus = () => {
      try {
        ...
        // Immediately hide onboarding if goals exist (prevent flash and navigation)
        // Only consider it a goal if it has a valid goalTitle
        const hasGoalData = (sessionUser && sessionUser.goalTitle && sessionUser.goalTitle.trim() !== '') || 
                           (goalsList.length > 0 && goalsList[0]?.goalTitle && goalsList[0].goalTitle.trim() !== '');
        if (hasGoalData) {
          // CRITICAL: Only set to false if it's currently true - prevents unnecessary re-renders
          if (showOnboarding) {
            setShowOnboarding(false);
          }
        } else {
          // No valid goal - show onboarding (but only if we're sure there's no data)
          // Check localStorage first to prevent false positives
          const localSession = typeof window !== 'undefined' ? localStorage.getItem('dreamscale_session') : null;
          if (localSession) {
            try {
              const parsed = JSON.parse(localSession);
              const localGoals = parsed?.hypeos?.allGoals || [];
              const localUser = parsed?.hypeos?.user;
              const hasLocalGoal = (localGoals.length > 0 && localGoals[0]?.goalTitle && localGoals[0].goalTitle.trim() !== '') ||
                                  (localUser?.goalTitle && localUser.goalTitle.trim() !== '');
              if (!hasLocalGoal) {
                setShowOnboarding(true);
              }
            } catch (e) {
              // If we can't parse, don't change onboarding state
            }
          } else {
            setShowOnboarding(true);
          }
        ...
        // Load all goals if available - CRITICAL: Always load, even if already set
        if (goalsList.length > 0) {
          console.log('✅ Loading', goalsList.length, 'goals from session:', goalsList.map((g: any) => g.goalTitle))
        ...
        // Load quests if available - CRITICAL: Always load from session
        const questsList = (sessionData?.hypeos as any)?.quests || [];
        if (questsList.length > 0) {
          console.log('✅ Loading', questsList.length, 'quests from session')
          setQuests(questsList);
        } else if (quests.length === 0 && hasGoalData) {
          // Initialize quests if they don't exist and user has a goal
          const initialQuests = initializeQuests();
          setQuests(initialQuests);
          console.log('✅ Initialized quests:', initialQuests.length, 'quests');
          updateHypeOSData({ quests: initialQuests });
        }
        
        // Check if tasks already exist from AI generation (don't overwrite them!)
        const existingTasksFromSession = (sessionData?.hypeos as any)?.tasks || [];
        const hasAIGeneratedTasks = existingTasksFromSession.length > 0;
        
        // Only generate tasks if user has goal but NO tasks exist (don't overwrite AI tasks!)
        if (hasGoalData && tasks.length === 0 && !hasAIGeneratedTasks) {
          const userForTasks = sessionUser || (goalsList.length > 0 ? goalsList[0] : null);
          if (userForTasks && userForTasks.goalTitle) {
            console.log('✅ Generating initial tasks for user with goal:', userForTasks.goalTitle);
            generateInitialTasks(userForTasks).catch(err => console.error('Error generating tasks:', err));
          }
        } else if (hasAIGeneratedTasks) {
          console.log('🔒 Preserving AI-generated tasks from Venture Quest:', existingTasksFromSession.length, 'tasks');
        }
        ...
        if (hasGoalData) {
          ...
          // IMPORTANT: Only load user if they have a goalTitle (actual goal created)
          const userToLoad = sessionUser || (goalsList.length > 0 ? goalsList[0] : null);
          if (userToLoad && userToLoad.goalTitle && userToLoad.goalTitle.trim() !== '') {
            ...
            // CRITICAL: localStorage ALWAYS has priority if it has today's tasks
            // This prevents session data from overwriting completed tasks
            if (savedTasksFromStorage && savedDateFromStorage === todayString) {
              // localStorage has today's tasks - ALWAYS use them, never overwrite
              try {
                const parsedLocal = JSON.parse(savedTasksFromStorage);
                const completedCount = parsedLocal.filter((t: any) => t.completed).length;
                console.log('✅ localStorage has today\'s tasks (ABSOLUTE PRIORITY), skipping session data:', {
                  completed: completedCount,
                  total: parsedLocal.length
                });
                
                // Load tasks from localStorage - preserve completed state
                const enrichedTasks = enrichTasksWithInstructions(parsedLocal);
                setTasks(enrichedTasks);
                
                // Load user data from localStorage to preserve points
                const savedUserFromStorage = typeof window !== 'undefined' ? localStorage.getItem('hypeos:user') : null;
                if (savedUserFromStorage) {
                  try {
                    const parsedUser = JSON.parse(savedUserFromStorage);
                    setUser(parsedUser);
                    console.log('✅ Loaded user from localStorage (with points):', parsedUser.hypePoints);
                  } catch (e) {
                ...
                hasLoadedFromSession.current = true;
                // Skip session data completely - localStorage is the source of truth
                return; // Exit early, don't check session data
              } catch (e) {
                console.warn('Failed to parse localStorage tasks', e);
                // Continue to session check below if parsing failed
              }
              ...
              if (tasksLastDateForLoading === todayString) {
                ...
                // Different day - ALWAYS regenerate tasks (like Duolingo - fresh tasks every day)
                console.log('🔄 Different day detected - regenerating tasks for new day', { tasksLastDateForLoading, todayString });
                // Always regenerate on new day - AI will try first, then fallback to generic
                generateInitialTasks(userWithOnboarding).catch(err => console.error('Error generating tasks:', err));
                updateHypeOSData({
              ...
              if (savedTasksFromStorage && savedDateFromStorage === todayString) {
                ...
                // Check if we have AI tasks from Venture Quest first
                const sessionAITasks = (sessionData?.hypeos as any)?.tasks || [];
                const sessionMiniWins = (sessionData?.hypeos as any)?.miniWins || [];
                const sessionQuests = (sessionData?.hypeos as any)?.quests || [];
                
                if (sessionAITasks.length > 0) {
                  console.log('✅ Using AI-generated tasks from Venture Quest:', sessionAITasks.length);
                  setTasks(sessionAITasks);
                  
                  // ALWAYS set mini wins and quests (even if empty, they should exist)
                  if (sessionMiniWins.length > 0) {
                    console.log('✅ Loading AI mini wins:', sessionMiniWins.length);
                    setMiniWins(sessionMiniWins);
                  } else {
                    console.warn('⚠️ No mini wins in session, but tasks exist - this shouldn\'t happen');
                  }
                  
                  if (sessionQuests.length > 0) {
                    console.log('✅ Loading AI quests:', sessionQuests.length);
                    setQuests(sessionQuests);
                  } else {
                    console.warn('⚠️ No quests in session, but tasks exist - this shouldn\'t happen');
                  }
                } else {
            ...
            // Always load miniWins if they exist
            // Load mini wins from session (CRITICAL: Always check for AI-generated ones first)
            const sessionMiniWins = (sessionData?.hypeos as any)?.miniWins || [];
            if (sessionMiniWins.length > 0) {
              setMiniWins(sessionMiniWins);
              console.log('✅ Loaded', sessionMiniWins.length, 'saved miniWins from session');
            } else {
              console.warn('⚠️ No mini wins found in session - this might be an issue');
            }
            // Quests are already loaded above in the goals section
          ...
          // CRITICAL: Skip localStorage check for authenticated users - data comes from Supabase
          // Only check localStorage for unauthenticated users
          let savedTasksFromStorage = null
          let savedDateFromStorage = null
          if (!authUser?.id) {
            savedTasksFromStorage = typeof window !== 'undefined' ? localStorage.getItem('hypeos:tasks') : null;
            savedDateFromStorage = typeof window !== 'undefined' ? localStorage.getItem('hypeos:tasksLastDate') : null;
          }
          
          // CRITICAL: localStorage ALWAYS has priority if it has today's tasks
          // This preserves user's completed tasks across refreshes - NEVER overwrite with session data
          if (savedTasksFromStorage && savedDateFromStorage === todayStringForGoal) {
            // localStorage has today's tasks - ALWAYS use them, never overwrite
            try {
              const parsedLocalTasks = JSON.parse(savedTasksFromStorage);
              const completedCount = parsedLocalTasks.filter((t: any) => t.completed).length;
              
              console.log('✅ localStorage has today\'s tasks (ABSOLUTE PRIORITY - goal), loading them:', {
                completed: completedCount,
                total: parsedLocalTasks.length
              });
              
              // Load tasks from localStorage - preserve completed state
              const enrichedTasks = enrichTasksWithInstructions(parsedLocalTasks);
              setTasks(enrichedTasks);
          ...
          // Only check session if localStorage doesn't have today's tasks
          if (!(savedTasksFromStorage && savedDateFromStorage === todayStringForGoal)) {
            if (sessionData?.hypeos?.tasks && sessionData.hypeos.tasks.length > 0) {
              ...
              if (savedTasksLastDateForGoal === todayStringForGoal) {
                ...
                // Different day - ALWAYS regenerate tasks (like Duolingo - fresh tasks every day)
                console.log('🔄 Different day detected (goal) - regenerating tasks for new day', { savedTasksLastDateForGoal, todayStringForGoal });
                // Always regenerate on new day - AI will try first, then fallback to generic
                generateInitialTasks(firstGoal).catch(err => console.error('Error generating tasks:', err));
                // Save the date tasks were generated
                updateHypeOSData({
              ...
              if (savedTasksFromStorage && savedDateFromStorage === todayStringForGoal) {
                ...
                if (sessionAITasks.length > 0) {
                  console.log('✅ Using AI-generated tasks from Venture Quest (goal):', sessionAITasks.length);
                  setTasks(sessionAITasks);
                  
                  // ALWAYS set mini wins and quests (even if empty, they should exist)
                  if (sessionMiniWins.length > 0) {
                    console.log('✅ Loading AI mini wins (goal):', sessionMiniWins.length);
                    setMiniWins(sessionMiniWins);
                  } else {
                    console.warn('⚠️ No mini wins in session (goal), but tasks exist');
                  }
                  
                  if (sessionQuests.length > 0) {
                    console.log('✅ Loading AI quests (goal):', sessionQuests.length);
                    setQuests(sessionQuests);
                  } else {
                    console.warn('⚠️ No quests in session (goal), but tasks exist');
                  }
                } else {
                  // No saved tasks anywhere - generate fresh tasks
          ...
          // Load mini wins (CRITICAL: Always check for AI-generated ones)
          const sessionMiniWinsForGoal = (sessionData?.hypeos as any)?.miniWins || [];
          if (sessionMiniWinsForGoal.length > 0) {
            setMiniWins(sessionMiniWinsForGoal);
            console.log('✅ Loaded', sessionMiniWinsForGoal.length, 'mini wins (goal)');
          } else {
            console.warn('⚠️ No mini wins found in session (goal)');
          }
    ...
    const timeoutId = setTimeout(() => {
      ...
      // Only consider it a goal if it has a goalTitle
      const hasGoals = goalsList.length > 0 && goalsList[0]?.goalTitle && goalsList[0].goalTitle.trim() !== '';
      const hasUserWithGoal = hasUser && hasUser.goalTitle && hasUser.goalTitle.trim() !== '';
      
      if (!hasUserWithGoal && !user && !hasGoals && allGoals.length === 0) {
        ...
        // Only load if goal has a valid goalTitle
        const firstGoal = goalsList[0];
        if (firstGoal && firstGoal.goalTitle && firstGoal.goalTitle.trim() !== '') {
          console.log('Found goals but user not set, loading from goals');
          const goalToLoad = {
            ...firstGoal,
            hasCompletedOnboarding: true,
            currentStreak: firstGoal.currentStreak ?? 0,
            longestStreak: firstGoal.longestStreak ?? 0
          };
          setUser(goalToLoad);
          setAllGoals(goalsList);
          hasLoadedFromSession.current = true;
          ...
          // Goal exists but no goalTitle - show onboarding
          setShowOnboarding(true);
        }
      } else if (user && user.goalTitle) {
        // If user is loaded and has a goal, make sure onboarding is hidden
        setShowOnboarding(false);
      } else if (user && !user.goalTitle) {
        // User exists but no goal - show onboarding
        setShowOnboarding(true);
      }
    ...
    (sessionData?.hypeos as any)?.quests?.length,
  ...
  useEffect(() => {
    ...
    if (tasksLastDate && tasksLastDate !== today && (tasks.length > 0 || hasSavedTasks)) {
      console.log('🔄 New day detected! Regenerating tasks (like Duolingo)...', { 
        tasksLastDate, 
        today, 
        currentTasksCount: tasks.length,
        hasSavedTasks 
      });
      // New day = regenerate tasks (AI will try first, then fallback to generic)
      // This ensures fresh tasks every day like Duolingo
      generateInitialTasks(user).catch(err => console.error('Error generating tasks:', err));
      ...
      // Same day but tasks are missing - check for AI tasks first, then localStorage
      const sessionAITasks = (sessionData?.hypeos as any)?.tasks || [];
      if (sessionAITasks.length > 0) {
        console.log('✅ Restoring AI-generated tasks from session:', sessionAITasks.length);
        setTasks(sessionAITasks);
        const sessionMiniWins = (sessionData?.hypeos as any)?.miniWins || [];
        const sessionQuests = (sessionData?.hypeos as any)?.quests || [];
        if (sessionMiniWins.length > 0) {
        ...
        return; // Don't check localStorage if we have AI tasks
      }
      
      // Only check localStorage for unauthenticated users if no AI tasks
      let savedTasksFromStorage = null
      let savedDateFromStorage = null
      if (!authUser?.id) {
        savedTasksFromStorage = typeof window !== 'undefined' ? localStorage.getItem('hypeos:tasks') : null;
      ...
      if (!authUser?.id && savedTasksFromStorage && savedDateFromStorage === today) {
  ...
  const completeTask = (taskId: number) => {
    ...
    // Update task completion - CRITICAL: Preserve all task properties
    ...
    // CRITICAL: Save tasks immediately to localStorage for ALL users (for persistence)
    // This ensures tasks persist even if session context hasn't updated yet
    try {
      if (typeof window !== 'undefined') {
    ...
    // CRITICAL: Save user points immediately to localStorage for ALL users (for persistence)
    try {
      if (typeof window !== 'undefined') {
        localStorage.setItem('hypeos:user', JSON.stringify(updatedUser));
        console.log('💾 Immediately saved user points to localStorage:', {
          oldPoints: user.hypePoints,
          newPoints: updatedUser.hypePoints,
          pointsEarned: completionResult.pointsEarned
        });
      }
    } catch (e) {
      ...
      try {
        ...
        try {
          if (typeof window !== 'undefined') {
            localStorage.setItem('hypeos:tasksLastDate', todayDate);
            localStorage.setItem('hypeos:tasks', JSON.stringify(updatedTasks));
            localStorage.setItem('hypeos:user', JSON.stringify(userToSave));
          }
        } catch (e) {
        ...
        // CRITICAL: Also save directly to dreamscale_session for immediate persistence
        // This ensures data persists across refreshes
        ...
        // Save to localStorage for ALL users (for persistence)
        try {
          if (typeof window !== 'undefined') {
            localStorage.setItem('dreamscale_session', JSON.stringify(updatedSession));
            console.log('✅ Tasks and user progress saved immediately to dreamscale_session:', {
              streak: userToSave.currentStreak,
              points: userToSave.hypePoints,
              completedTasks: updatedTasks.filter(t => t.completed).length,
              totalTasks: updatedTasks.length
            });
          }
        } catch (e) {
          console.warn('Failed to save to dreamscale_session', e);
        }
      } catch (error) {
    ...
    if (allTasksCompleted) {
      ...
      try {
        ...
        if (!authUser?.id) {
          localStorage.setItem('dreamscale_session', JSON.stringify(updatedSession));
          console.log('✅ Streak updated and saved (unauthenticated) -', userWithUpdatedStreak.currentStreak, 'day streak!');
        }
      } catch (error) {
    ...
    if (!authUser?.id) {
      try {
        if (typeof window !== 'undefined') {
          ...
          console.log('💾 Immediately saved completed miniWin to localStorage (unauthenticated)');
        }
      } catch (e) {
    ...
    if (updatedUser) {
      ...
      if (!authUser?.id) {
        try {
          if (typeof window !== 'undefined') {
            localStorage.setItem('hypeos:user', JSON.stringify(updatedUser));
            console.log('💾 Immediately saved user (miniWin) to localStorage (unauthenticated)');
          }
        } catch (e) {
          console.warn('Failed to immediately save user to localStorage', e);
        }
    ...
    if (sessionContext && updatedUser) {
      try {
        ...
        if (!authUser?.id) {
          ...
          localStorage.setItem('dreamscale_session', JSON.stringify(updatedSession));
          console.log('✅ MiniWins saved immediately to localStorage (unauthenticated)');
        }
      } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Loading Venture Quest...</p>
        </div>
      </div>
    );
  }

  // CRITICAL: Show onboarding when user explicitly clicks "New Goal" OR when no goals exist
  const shouldShowOnboarding = () => {
    // If still loading on initial page load, don't show yet
    if (isLoading && !showOnboarding) {
      return false;
    }
    
    // IMPORTANT: If user explicitly clicked "New Goal" button, ALWAYS show onboarding
    // This allows creating multiple goals
    if (showOnboarding) {
      return true;
    }
    
    // Only auto-show onboarding if no goals exist (initial state)
    // Check if we have goal data in localStorage
    if (typeof window !== 'undefined') {
      try {
        const stored = localStorage.getItem('dreamscale_session');
        if (stored) {
          const parsed = JSON.parse(stored);
          const goalsList = parsed?.hypeos?.allGoals || [];
          const userData = parsed?.hypeos?.user;
          const hasValidGoal = (goalsList.length > 0 && goalsList[0]?.goalTitle && goalsList[0].goalTitle.trim() !== '') ||
                              (userData?.goalTitle && userData.goalTitle.trim() !== '');
          if (hasValidGoal) {
            return false; // Don't auto-show onboarding if we have goals
          }
        }
      } catch (e) {
        // Ignore errors
    ...
    // Check state - if user has goal, don't auto-show
    if (user && user.goalTitle) {
      return false;
    }
    
    if (allGoals.length > 0 && allGoals[0]?.goalTitle) {
      return false;
    }
    
    // No goals exist - show onboarding for first-time setup
    return true;
  };
    ...
    <div className="hypeos-page min-h-screen bg-white dark:bg-slate-950" style={{ overflowY: 'auto', height: 'auto', minHeight: '100vh' }}>
      ...
      <div className="max-w-7xl mx-auto px-8 py-16 pt-8">
        ...
        <div className="mb-12">
          ...
          {/* Stats Row - Only show if user has a goal */}
          {user && user.goalTitle && (
            <div className="flex items-center gap-6 pb-4 border-b border-gray-200 dark:border-gray-800">
              <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-500">
                <Flame className="h-4 w-4" />
                <span>Streak {user.currentStreak || 0} days</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-500">
                <Star className="h-4 w-4" />
                <span>Level {getLevelFromPoints(user?.hypePoints || 0).level}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-500">
                <Zap className="h-4 w-4" />
                <span>Points {user.hypePoints || 0}</span>
              </div>
            </div>
          )}
        </div>

        {/* Conditional Content Based on View Mode - Only show if user has a goal */}
        {user && user.goalTitle && viewMode === 'default' ? (
          <>
            {/* Daily Goal Section */}
            {dailyGoal && (() => {
              const userId = sessionData?.email || user?.name || 'default-user';
              ...
              const todayXP = history.lastWeekXP.length > 0 
                ? history.lastWeekXP[history.lastWeekXP.length - 1] 
                : 0;
              const currentTasks = tasks.filter(t => t.completed).length;
              const progress = calculateGoalProgress(todayXP, currentTasks, dailyGoal);
              const difficultyInfo = getDifficultyInfo(dailyGoal.difficulty);
              
              return (
                <div className="mb-12">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Daily Goal</h2>
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-gray-500 dark:text-gray-500">{difficultyInfo.name}</span>
                      <Settings className="h-4 w-4 text-gray-500 dark:text-gray-500" />
                    </div>
                  </div>
                  
                  {/* Info box - white in light mode, dark in dark mode */}
                  <div className="bg-white dark:bg-blue-900/20 border border-gray-200 dark:border-blue-700/50 rounded-lg p-4 mb-6">
                    <p className="text-sm text-gray-700 dark:text-blue-200">
                      Based on your recent activity, this goal fits your pace.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-sm font-semibold text-gray-900 dark:text-white">{Math.round(progress.completionPercentage)}%</span>
                      </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="text-right">
                          <span className="text-sm font-semibold text-gray-900 dark:text-white">{todayXP}/{dailyGoal.targetXP}</span>
                          {progress.xpRemaining > 0 && (
                            <span className="text-xs text-gray-500 dark:text-gray-500 ml-1">{progress.xpRemaining} XP remaining</span>
                          )}
                        </div>
                      </div>
                      <Progress value={Math.min((todayXP / dailyGoal.targetXP) * 100, 100) || 0} className="h-2" />
                    </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="text-right">
                          <span className="text-sm font-semibold text-gray-900 dark:text-white">{currentTasks}/{dailyGoal.targetTasks}</span>
                          {progress.tasksRemaining > 0 && (
                            <span className="text-xs text-gray-500 dark:text-gray-500 ml-1">{progress.tasksRemaining} tasks remaining</span>
                          )}
                        </div>
                      </div>
                      <Progress value={Math.min((currentTasks / dailyGoal.targetTasks) * 100, 100) || 0} className="h-2" />
                    </div>
            ...
            {/* Two Column Layout: Tasks on Left, Streak on Right */}
            <div className="grid grid-cols-1 lg:grid-cols-7 gap-6">
              {/* Left Column: Tasks and Mini Wins - Larger */}
              ...
              {/* Right Column: Streak and Progress - Smaller */}
              <div className="lg:col-span-2 space-y-4 flex flex-col max-w-xs">
                {/* Streak Card */}
                <StreakTracker
                  currentStreak={user?.currentStreak || 0}
                  longestStreak={user?.longestStreak || 0}
                ...
                {/* Progress Card */}
                {(() => {
                  const userPoints = user?.hypePoints || 0;
                  const levelInfo = getLevelFromPoints(userPoints);
                  
                  if (!dailyGoal) {
                    // Calculate monthly progress even when no daily goal
                    const goalTarget = user?.goalTarget || "15000/m";
                    const isMonthlyGoal = goalTarget.includes('/m');
                    
                    let goalProgressPercent = 0;
                    let goalCurrent = '0';
                    
                    if (isMonthlyGoal) {
                      // Extract monthly target number (e.g., "15000/m" -> 15000)
                      const monthlyTargetMatch = goalTarget.match(/(\d+)/);
                      const monthlyTarget = monthlyTargetMatch ? parseInt(monthlyTargetMatch[1]) : 15000;
                      
                      // Use total user points as monthly progress
                      goalProgressPercent = Math.min(100, (userPoints / monthlyTarget) * 100);
                      goalCurrent = `${userPoints}`;
                    }
                    ...
                    return (
                      <ProgressCard
                        ...
                        nextLevelPoints={levelInfo.pointsToNext + userPoints}
                        streak={user?.currentStreak || 0}
                  ...
                  // Check if goal is monthly (has "/m" in target)
                  const goalTarget = user?.goalTarget || `${dailyGoal.targetXP}/m`;
                  const isMonthlyGoal = goalTarget.includes('/m');
                  ...
                  if (isMonthlyGoal) {
                    ...
                    const currentTasks = tasks.filter(t => t.completed).length;
                    const progress = calculateGoalProgress(todayXP, currentTasks, dailyGoal);
                    goalProgressPercent = progress.completionPercentage || 0;
                    goalCurrent = `${currentTasks}/${dailyGoal.targetTasks}`;
                  }
                  
                  // Calculate next level progress using proper level system
                  // levelInfo is already declared at the top of this function
                  const currentLevel = levelInfo.level;
                  const nextLevelProgress = levelInfo.progressPercentage;
                  const pointsForNextLevel = levelInfo.pointsToNext + userPoints; // Total points needed for next level
            ...
            <Card className="bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-gray-800 shadow-sm">
              <CardHeader>
                <CardTitle className="flex items-center space-x-2 text-lg font-semibold text-gray-900 dark:text-white">
                  ...
                  <span>All Your Venture Quest Goals</span>
                </CardTitle>
              </CardHeader>
              <CardContent>
                {allGoals.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {allGoals
                      .filter(goal => goal && goal.goalTitle && goal.goalTitle.trim() !== '')
                      .map((goal, index) => {
                      const goalId = goal.goalTitle + '-' + goal.category;
                      ...
                      return (
                        <Card 
                          ...
                          className={`cursor-pointer transition-all duration-200 hover:shadow-md border ${
                            isActive 
                              ? 'border-2 border-gray-900 dark:border-white bg-gray-50 dark:bg-gray-800' 
                          ...
                          <CardContent className="p-5">
                            <div className="flex items-start justify-between mb-4">
                              <div className="flex-1">
                                ...
                                <Badge className="bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300 border-0 font-medium text-xs">
                                  {goal.category || 'General'}
                                </Badge>
                              </div>
                              {isActive && (
                                <Badge className="bg-gray-900 text-white dark:bg-gray-800 dark:text-white dark:border dark:border-gray-700 border-0 font-medium text-xs">
                                  Active
                                </Badge>
                              )}
                            </div>
                            ...
                            <div className="space-y-2 text-sm">
                              ...
                              <Progress value={goal.goalProgress || 0} className="h-1.5" />
                              ...
                              <div className="flex items-center justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
                                ...
                                <div className="flex items-center space-x-1.5">
                                  <Flame className="h-3.5 w-3.5 text-gray-500 dark:text-gray-400" />
                                  <span className="text-xs text-gray-600 dark:text-gray-400">
                            ...
                            {!isActive && (
                              <Button 
                                variant="outline" 
                                size="sm" 
                                className="w-full mt-4 !text-gray-900 dark:!text-gray-300 border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-900 font-medium"
                                onClick={(e) => {
                                  e.preventDefault();
                                  e.stopPropagation();
                                  switchToGoal(goal);
