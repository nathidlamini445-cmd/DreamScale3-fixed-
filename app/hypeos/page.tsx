...
export default function HypeOSPage() {
  ...
  useEffect(() => {
    ...
    const checkUserStatus = () => {
      try {
        ...
        // Ensure tasks are generated if user has goal but no tasks
        if (hasGoalData && tasks.length === 0) {
          ...
          if (userForTasks && userForTasks.goalTitle) {
            ...
            generateInitialTasks(userForTasks);
          }
        }
  ...
  useEffect(() => {
    ...
    if (tasksLastDate && tasksLastDate !== today && (tasks.length > 0 || hasSavedTasks)) {
      ...
      // Same day but tasks are missing - only check localStorage for unauthenticated users
      let savedTasksFromStorage = null
      let savedDateFromStorage = null
      if (!authUser?.id) {
        savedTasksFromStorage = typeof window !== 'undefined' ? localStorage.getItem('hypeos:tasks') : null;
  ...
  const handleOnboardingComplete = (userData: User) => {
    ...
    try {
      ...
      if (!authUser?.id) {
        localStorage.setItem('dreamscale_session', JSON.stringify(updatedSession));
        console.log('✅ Goal saved immediately to localStorage (unauthenticated)');
      }
    } catch (error) {
  ...
  // CRITICAL: Only show onboarding if we're absolutely sure there's no goal data
  // This prevents navigation flicker on page refresh
  // Check both user state and localStorage before showing onboarding
  const shouldShowOnboarding = () => {
    if (showOnboarding && isLoading) {
      // Still loading - don't show onboarding yet (prevents navigation)
      return false;
    }
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        if (stored) {
          ...
          if (hasValidGoal) {
            return false; // Don't show onboarding if we have goals
          }
        }
      } catch (e) {
        // Ignore errors
    ...
    if (user && user.goalTitle) {
      return false; // User has goal
    }
    
    if (allGoals.length > 0 && allGoals[0]?.goalTitle) {
      return false; // Goals exist
    }
    
    return showOnboarding; // Use the state value
  };
