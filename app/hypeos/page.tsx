                ...
                {(() => {
                  if (!dailyGoal) {
                    return (
                      <ProgressCard
                        level={user?.level || 1}
                        currentPoints={user?.hypePoints || 0}
                        goalProgress={0}
                        goalCurrent="0"
                        goalTarget="15000/m"
                        nextLevelProgress={0}
                  ...
                  const goalProgressPercent = progress.completionPercentage || 0;
                  
                  // Calculate next level progress (points towards next level)
                  const currentLevel = user?.level || 1;
                  const pointsForNextLevel = currentLevel * 100; // Example: 100 points per level
                  const nextLevelProgress = user?.hypePoints ? (user.hypePoints % pointsForNextLevel) / pointsForNextLevel * 100 : 0;
                  
                  // Format goal current/target - use user's goal target if available
                  const goalCurrent = `${currentTasks}/${dailyGoal.targetTasks}`;
