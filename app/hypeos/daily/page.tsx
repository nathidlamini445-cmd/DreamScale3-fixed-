...
export default function DailyFocusPage() {
  ...
  useEffect(() => {
    try {
      ...
      try {
        // First try session context (for authenticated users)
        const sessionUser = sessionContext?.sessionData?.hypeos?.user || 
                          sessionContext?.sessionData?.hypeos?.allGoals?.[0];
        if (sessionUser) {
          setUserGoal({
          ...
          if (stored) {
            ...
            const localStorageUser = parsed?.hypeos?.user || parsed?.hypeos?.allGoals?.[0];
            if (localStorageUser) {
              setUserGoal({
                goalTitle: localStorageUser.goalTitle,
                category: localStorageUser.category,
                goalTarget: localStorageUser.goalTarget
              });
            }
          }
        }
  ...
  const generateAITasks = async () => {
    ...
    try {
      ...
      if (!generatedTasks || generatedTasks.length === 0) {
        console.warn('No tasks generated');
        alert('No tasks could be generated. Please make sure you have a goal set.');
        setIsGeneratingTasks(false);
        return;
      }
      
      console.log('✅ Generated tasks:', generatedTasks.length, generatedTasks);
      ...
      // Save to session context (for authenticated users) and localStorage
      try {
        if (sessionContext?.updateHypeOSData) {
          // Save to Supabase via session context
          sessionContext.updateHypeOSData({ tasks: updatedTasks });
          console.log('💾 Saved generated tasks to Supabase via session context:', newTasks.length);
        }
        
        // Also save to localStorage (for unauthenticated users or backup)
        if (typeof window !== 'undefined') {
          const today = new Date().toDateString();
        ...
        console.warn('Failed to save generated tasks:', e);
      }
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-8 py-16 pt-8">
        ...
        <div className="mt-8 flex justify-center">
          <button
            ...
            className="px-6 py-3 rounded-lg border-2 text-white hover:bg-opacity-80 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 cursor-pointer"
