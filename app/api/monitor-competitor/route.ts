...
export async function POST(request: Request) {
  try {
    // Rate limit: 10 monitoring runs per minute per client.
    const rateLimited = rateLimit(`monitor-competitor:${getClientIp(request)}`, 10, 60_000)
    ...
    // Cap the per-request batch to avoid amplification attacks.
    const MAX_URLS = 25
    if (competitor_urls.length > MAX_URLS) {
      return NextResponse.json(
        { error: `Too many URLs in one request (max ${MAX_URLS})` },
        { status: 400 }
      )
    }

    // SSRF guard each URL before doing anything with it.
    const validatedUrls: string[] = []
    for (const u of competitor_urls) {
      if (typeof u !== 'string') continue
      const safe = await assertPublicHttpUrl(u)
      if (safe.ok) validatedUrls.push(safe.url.toString())
    }
    if (validatedUrls.length === 0) {
      return NextResponse.json(
        { error: 'No valid public URLs supplied' },
        { status: 400 }
      )
    }

    console.log(`🔍 Starting competitor monitoring for ${validatedUrls.length} competitors`)
    ...
    return NextResponse.json({
      ...
      summary: {
        total_competitors: validatedUrls.length,
        successful_monitors: monitoringResults.filter(
          r => r.alerts.length === 0 || !r.alerts.some(a => a.message.includes('Failed'))
        ).length,
        high_priority_alerts: monitoringResults
          .flatMap(r => r.alerts)
