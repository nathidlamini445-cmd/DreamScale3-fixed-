...
import { taskGenerateSchema } from '@/lib/validations'

export const maxDuration = 60

export async function POST(request: NextRequest) {
  try {
    // Rate limit: 5 task generations per minute per client
    const rateLimited = rateLimit(`tasks-gen:${getClientIp(request)}`, 5, 60_000)
    if (rateLimited) return rateLimited

    const body = await request.json()
    const parsed = taskGenerateSchema.safeParse(body)
