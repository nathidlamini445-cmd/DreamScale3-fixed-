...
export async function POST(request: NextRequest) {
  try {
    const quotaGate = await requireMonthlyQuota('teams')
...
Return ONLY valid JSON, no markdown formatting, no code blocks.`
    ...
    try {
      ...
      // Provide comprehensive fallback structure
      ritualData = {
        duration: 30,
        purpose: `A ${frequency} ${type.replace('-', ' ')} ritual for ${name} to maintain team alignment, communication, and productivity.`,
        participants: teamSize ? [`Team of ${teamSize}`] : ['All team members'],
        preparation: {
          beforehand: ['Prepare agenda', 'Review previous notes', 'Set up meeting space'],
          materials: ['Notebook', 'Agenda document'],
          setup: 'Ensure meeting space is ready and all participants are notified'
        },
        structure: {
          sections: [
            { 
              name: 'Opening', 
              duration: 5, 
              description: 'Quick check-in and agenda review',
              activities: ['Welcome participants', 'Review agenda', 'Set expectations'],
              outcomes: ['Everyone aligned on agenda'],
              tips: ['Start on time', 'Keep it brief']
            },
            { 
              name: 'Main Discussion', 
              duration: 20, 
              description: 'Core agenda items and discussion',
              activities: ['Discuss key topics', 'Address blockers', 'Share updates'],
              outcomes: ['Issues identified', 'Decisions made'],
              tips: ['Stay focused', 'Encourage participation']
            },
            { 
              name: 'Action Items', 
              duration: 5, 
              description: 'Review and assign action items',
              activities: ['Summarize decisions', 'Assign tasks', 'Set deadlines'],
              outcomes: ['Clear action items', 'Owners assigned'],
              tips: ['Be specific', 'Set deadlines']
            }
          ]
        },
        aiSuggestions: {
          whyNeeded: `This ${type.replace('-', ' ')} ritual is essential for maintaining team cohesion, ensuring everyone is aligned on goals, and addressing blockers quickly. Regular ${frequency} check-ins help prevent miscommunication and keep projects on track.`,
          bestPractices: [
          ...
          adaptations: [],
          successFactors: ['Consistency', 'Clear agenda', 'Active participation'],
          measuringSuccess: ['Action items completed', 'Team satisfaction', 'Project progress']
        },
        followUp: {
          immediate: ['Send meeting notes', 'Update project tracker'],
          afterRitual: ['Follow up on action items', 'Schedule next meeting'],
          longTerm: ['Review ritual effectiveness', 'Adjust as needed']
        },
        variations: [],
        resources: ['Meeting template', 'Action item tracker'],
        troubleshooting: [
          {
            issue: 'Meeting runs over time',
            solution: 'Use a timer and stick to agenda',
            prevention: 'Plan agenda items with time limits'
          }
        ]
      }
    }
