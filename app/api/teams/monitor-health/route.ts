...
export async function POST(request: NextRequest) {
  try {
    ...
    const { teamName, teamSize, currentChallenges, recentChanges, teamConcerns } = await request.json()
    ...
    if (currentChallenges) contextParts.push(`Current Challenges: ${currentChallenges}`)
    if (recentChanges) contextParts.push(`Recent Changes: ${recentChanges}`)
    if (teamConcerns) contextParts.push(`Team Concerns: ${teamConcerns}`)
    const contextString = contextParts.length > 0 ? `\n\nAdditional Context:\n${contextParts.join('\n')}` : ''

    const prompt = `You are TeamSync AI, an expert in team health monitoring and organizational wellness.

Analyze the health of this team: ${teamName}${contextString}
