...
import { welcomeEmailSchema } from '@/lib/validations'
        ...
        contents: [
          {
            parts: [
              {
                text: `Generate a friendly and professional HTML welcome email for DreamScale (a productivity app with features like calendar events, Venture Quest goals, and Bizora AI chats). 
                
The user's email is: ${email}

The email should:
1. Be warm and welcoming
2. Highlight key features (Calendar, Venture Quest goals, Bizora AI)
...
Format: Return ONLY the HTML content, wrapped in a container div with styling. Include company branding.`
    ...
    // Fallback template if no content generated
    return `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 12px; color: white;">
        <div style="background: white; color: #333; padding: 30px; border-radius: 8px; margin-bottom: 20px;">
          <h1 style="color: #39d2c0; margin: 0 0 20px 0;">Welcome to DreamScale! 🚀</h1>
...
export async function POST(req: NextRequest) {
  try {
    // Rate limit: 3 emails per minute per client
    const rateLimited = rateLimit(`welcome-email:${getClientIp(req)}`, 3, 60_000)
    if (rateLimited) return rateLimited

    // CRITICAL: only authenticated users can trigger an email and only to
    // their own address — otherwise this is an open email relay.
    ...
    if (
      !auth.user.email ||
      email.toLowerCase().trim() !== auth.user.email.toLowerCase().trim()
    ) {
      return NextResponse.json(
        { error: 'Forbidden: can only send welcome email to your own address' },
        { status: 403 }
      )
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('Welcome email generated (dev mode)')
    }
