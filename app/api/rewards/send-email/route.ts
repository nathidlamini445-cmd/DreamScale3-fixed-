...
import { rewardEmailSchema } from '@/lib/validations';
...
export async function POST(req: NextRequest) {
  try {
    // Rate limit: 5 reward emails per minute per client
    const rateLimited = rateLimit(`reward-email:${getClientIp(req)}`, 5, 60_000);
    ...
    const parsed = rewardEmailSchema.safeParse(body);
    ...
    const { rewardId, email, userName, downloadLink, rewardTitle } = parsed.data;
    ...
    if (
      ...
      return NextResponse.json(
        { error: 'Forbidden: can only email your own address' },
        { status: 403 }
      );
    }
    ...
    if (!emailSent && process.env.NODE_ENV === 'development') {
      console.log('Reward email generated (email service not configured)');
    }
