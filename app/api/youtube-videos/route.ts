...
async function searchYouTube(query: string, maxResults: number = 5): Promise<any[]> {
  try {
    // Server-only key. Never read NEXT_PUBLIC_YOUTUBE_API_KEY here — that
    // would expose the key to anyone who inspects the client bundle.
...
async function getVideoDetails(videoIds: string[]): Promise<Record<string, { duration: string; views: string }>> {
  try {
    ...
    const response = await fetch(detailsUrl, { signal: AbortSignal.timeout(15_000) })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const useDynamicSearch = !!process.env.YOUTUBE_API_KEY
