...
export async function POST(request: NextRequest) {
  try {
    const { message, isResearch = false, fileContent, userProfile, dailyMood, hobbies, favoriteSong } = await request.json()
    ...
    if (dailyMood) {
      ...
      if (dailyMood === 'demotivated' || dailyMood === 'tired' || dailyMood === 'overwhelmed') {
        ...
        personalizationContext += `\nMOOD ADJUSTMENT: The user is feeling ${dailyMood}. Be calming, prioritize simplification, focus on one thing at a time. Help them feel more in control. Provide clear, actionable steps without overwhelming them.${favoriteSong ? ` Consider suggesting they take a break and listen to "${favoriteSong}" to help them relax and refocus.` : ''}\n`
      }
    }
