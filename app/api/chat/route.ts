...
export async function POST(request: NextRequest) {
  try {
    ...
    const { message, conversationHistory = [], isResearch = false, fileContent, fileAttachments, userProfile: clientUserProfile, dailyMood, hobbies, favoriteSong, userId } = body
    
    // CRITICAL: If userProfile is missing or incomplete, fetch from Supabase user_profiles table
    let userProfile = clientUserProfile
    
    // Fetch from Supabase if userId is provided and userProfile is missing/incomplete
    if (userId && (!userProfile || !userProfile.businessName)) {
      try {
        const supabase = await createClient()
        
        console.log('🔍 Fetching user business data from Supabase user_profiles for AI')
        ...
        if (error) {
          ...
          console.log('✅ Loaded user business data from Supabase for AI:', {
    ...
    console.log('Gemini API request:', { 
      ...
      conversationHistoryLength: safeConversationHistory.length, 
      isResearch, 
      hasFileContent: !!fileContent, 
      hasFileAttachments: !!fileAttachments, 
    ...
    // Use hobbies from userProfile if available, otherwise use passed hobbies
    const userHobbies = (userProfile?.hobbies && userProfile.hobbies.length > 0) ? userProfile.hobbies : (hobbies || [])
    
    if (userHobbies && userHobbies.length > 0) {
      personalizationContext += `- Hobbies/Interests: ${userHobbies.join(', ')}\n`
      personalizationContext += `\nCRITICAL - HOBBIES INTEGRATION: The user's hobbies (${userHobbies.join(', ')}) are an important part of their identity and should be mentioned regularly in your responses. Use their hobbies in multiple ways:\n`
      ...
      personalizationContext += `2. Reference their hobbies when providing motivation or encouragement (e.g., "Think about how you approach ${userHobbies[0]} - you probably started as a beginner and improved through practice. The same principle applies to building your business.")\n`
      ...
      personalizationContext += `5. When they're feeling demotivated, stressed, or overwhelmed, suggest taking a break to engage in their hobbies (${userHobbies.join(' or ')}) as a way to recharge, similar to how you'd suggest their favorite song.\n`
    ...
    if (dailyMood) {
      ...
      if (dailyMood === 'demotivated' || dailyMood === 'tired' || dailyMood === 'overwhelmed') {
        const hobbySuggestion = userHobbies && userHobbies.length > 0 ? ` Consider suggesting they take a break to engage in ${userHobbies[0]}${userHobbies.length > 1 ? ` or ${userHobbies[1]}` : ''} to recharge and reset their mindset.` : ''
        ...
        personalizationContext += `\nMOOD ADJUSTMENT: The user is feeling ${dailyMood}. They're ready to push forward! Challenge them with bigger goals, suggest scaling opportunities, advanced strategies, and help them discover new ways to grow. Be enthusiastic and match their energy.${userHobbies && userHobbies.length > 0 ? ` Use their hobbies (${userHobbies.join(', ')}) to create energizing analogies that match their current motivation level.` : ''}\n`
      } else if (dailyMood === 'stressed' || dailyMood === 'anxious' || dailyMood === 'uncertain') {
        const hobbySuggestion = userHobbies && userHobbies.length > 0 ? ` Consider suggesting they take a break to engage in ${userHobbies[0]}${userHobbies.length > 1 ? ` or ${userHobbies[1]}` : ''} to help them relax and refocus.` : ''
