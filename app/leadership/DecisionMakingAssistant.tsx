...
import { Loader2, X, Share } from "lucide-react"
...
export default function DecisionMakingAssistant({ decisions, onAddDecision }: DecisionMakingAssistantProps) {
  ...
  useEffect(() => {
    if (navigatingId) {
      ...
      if (pathname && pathname.startsWith('/leadership/decision/')) {
  ...
  const formatDecisionForSharing = (decision: Decision): string => {
    let content = `# Decision Analysis: ${decision.description}\n\n`
    
    if (decision.context) {
      content += `## Context\n\n${decision.context}\n\n`
    }
    
    if (decision.analysis?.overview) {
      content += `## Overview\n\n${decision.analysis.overview}\n\n`
    }
    
    if (decision.analysis?.frameworks && decision.analysis.frameworks.length > 0) {
      content += `## Decision Frameworks\n\n`
      decision.analysis.frameworks.forEach((framework, index) => {
        content += `### ${framework.name}\n\n${framework.analysis}\n\n`
      })
    }
    
    if (decision.analysis?.pros && decision.analysis.pros.length > 0) {
      content += `## Pros\n\n`
      decision.analysis.pros.forEach(pro => {
        content += `- ${pro}\n`
      })
      content += `\n`
    }
    
    if (decision.analysis?.cons && decision.analysis.cons.length > 0) {
      content += `## Cons\n\n`
      decision.analysis.cons.forEach(con => {
        content += `- ${con}\n`
      })
      content += `\n`
    }
    
    if (decision.analysis?.recommendation) {
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  onClick={() => {
                    if (selectedDecision) {
                      const content = formatDecisionForSharing(selectedDecision)
                      setShareModal({
                        isOpen: true,
                        content,
                        title: `Decision Analysis: ${selectedDecision.description}`
                      })
                    }
                  }}
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <Button 
                      ...
                      onClick={(e) => {
                        ...
                        router.push(`/leadership/decision/${decision.id}`)
                      }}
                      disabled={navigatingId === decision.id}
                    >
                      {navigatingId === decision.id ? (
                        <>
                          <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                          Opening...
                        </>
