...
import {
  formatPricingForShare,
  formatPricingForSheet,
      ...
      try {
        ...
        if (user?.id) {
          try {
            ...
            if (dbData?.pricingStrategies) {
              const found = dbData.pricingStrategies.find((s: PricingStrategy) => s.id === id)
              if (found) {
                setStrategy(found)
                setIsLoading(false)
                return
          ...
          <div className="max-w-6xl px-12 py-10">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              <RevenueShareBar
                ...
                contentType="Revenue · Pricing"
                textContent={formatPricingForShare(strategy)}
                sheetExport={formatPricingForSheet(strategy)}
              />
            </div>
          </div>
        </div>
