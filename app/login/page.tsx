  ...
  useEffect(() => {
    ...
    if (err === 'session_check_failed' || decoded.includes('session_unavailable')) {
      ...
      alert(
        ...
        'Your session took too long to load or cookies were blocked. Refresh and sign in again.\n\nGoogle sign-in tip: Supabase Dashboard → Authentication → URL Configuration → add BOTH http://localhost:3000/auth/callback and http://127.0.0.1:3000/auth/callback, then always use ONE address in the browser (do not mix localhost and 127.0.0.1).',
    ...
    if (err === 'missing_supabase_env' || decoded.includes('missing_supabase_env')) {
      ...
      alert(
        ...
        'This app cannot find Supabase. Add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to .env.local (see .env.example), then restart the dev server.',
      ...
      alert(
        ...
        `Sign-in did not finish: ${decoded}\n\nIf Continue with Google returns you here, verify redirect URLs in Supabase and use localhost OR 127.0.0.1 consistently—not both in the same flow.`,
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          alert(augmentCaptchaAlertMessage(`Could not create account: ${error.message}`, captchaConfigured))
        }
