...
/** Local dev: fail fast. Production: more time for long-haul links + cold Supabase Auth. */
function getAuthHttpTimeoutMs(): number {
  if (typeof window === 'undefined') return 60_000
  const h = window.location.hostname
  if (h === 'localhost' || h === '127.0.0.1') return 22_000
  return 60_000
}

async function withNetworkTimeout<T>(promise: Promise<T>, ms: number, label: string): Promise<T> {
  ...
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(
      () =>
        reject(
          new Error(
            `${label} timed out after ${ms / 1000}s — check your connection, VPN, and that your Supabase project is running.`,
          ),
        ),
      ms,
    )
...
function LoginPageContent() {
  ...
  useEffect(() => {
    ...
    if (err === 'session_check_failed' || decoded.includes('session_unavailable')) {
      ...
      setAuthBanner({
        ...
        detail:
          'Your session took too long or cookies may be blocked. Try a refresh, another browser, or disable extensions that block third-party cookies.\n\nSupabase → Authentication → URL Configuration: add your Site URL and Redirect URLs, including https://app.dreamscale.co.za/auth/callback (production) and your local callback if you develop locally (use either localhost or 127.0.0.1 consistently, not both).',
      })
  ...
  const handleEmailSignIn = async () => {
    ...
    try {
      ...
      const authTimeoutMs = getAuthHttpTimeoutMs()
      const runSignIn = () =>
        withNetworkTimeout(
      ...
      try {
        ...
        const isTimeout =
          first instanceof Error && first.message.toLowerCase().includes('timed out')
        if (isTimeout) {
          console.warn('[login] Sign-in timed out — retrying once')
          const result = await runSignIn()
          data = result.data
          error = result.error
        } else {
      ...
      setAuthBanner({
        ...
        detail: `${msg}\n\nTry again, use Google, or check Supabase Dashboard → Logs if this keeps happening.`,
