...
export default function TaskAssignmentDetailPage() {
      ...
      try {
        ...
        if (user?.id) {
          try {
            ...
            if (dbData?.taskAssignments) {
              const foundAssignment = dbData.taskAssignments.find((a: SmartTaskAssignment) => a.id === params.id)
              if (foundAssignment) {
  ...
  if (!assignment) {
    return (
      <div className="min-h-screen bg-white dark:bg-gray-800 text-foreground overflow-y-auto">
        <SidebarNav />
