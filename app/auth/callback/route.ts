...
export async function GET(request: NextRequest) {
  ...
  if (code) {
    ...
    try {
      ...
      if (!user) return redirectOnSuccess
      ...
      if (existingErr) {
        console.warn('⚠️ [CALLBACK] Profile lookup (non-blocking):', existingErr.message)
      ...
      if (!existing) {
        ...
        if (insertError) {
          console.warn('⚠️ [CALLBACK] Profile insert (non-blocking):', insertError.message)
        }
      } else {
        const { error: updateError } = await supabase
          ...
          .eq('id', user.id)
        if (updateError) {
          console.warn('⚠️ [CALLBACK] Profile update (non-blocking):', updateError.message)
        }
      }
    } catch (e) {
