...
export default function DiscoverPage() {
      ...
      try {
        ...
        if (data.videos && Array.isArray(data.videos)) {
          ...
          const convertedVideos: ContentItem[] = data.videos.map((v: any, index: number) => {
            ...
            const thumbnail = v.thumbnail || `https://img.youtube.com/vi/${v.videoId}/maxresdefault.jpg`
  ...
  const allVideos = useMemo(() => {
    ...
    const personalized = personalizeContent(combined, userProfile)
    
    // Calculate scores to identify highly personalized content
    const scoredVideos = personalized.map((video, index) => {
      // First 6 videos are considered "highly personalized" (top matches)
      return {
        ...video,
        _isPersonalized: index < 6,
        _personalizedRank: index + 1
      }
    })
    
    return scoredVideos
  }, [industryContent, userProfile, refreshKey, youtubeVideos])
  ...
  const allStories = useMemo(() => {
    const baseStories = industryContent?.entrepreneurStories && industryContent.entrepreneurStories.length > 0
      ...
      : entrepreneurStories
    return personalizeContent(baseStories, userProfile)
  }, [industryContent, userProfile, refreshKey])
              ...
              <div className="xl:col-span-3 space-y-8">
                ...
                {!isLoadingVideos && displayVideos.length > 0 && (
                  <div className="p-6 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:shadow-md transition-all duration-200 relative">
                    {displayVideos[0]._isPersonalized && (
                      <Badge className="absolute top-4 right-4 bg-primary text-primary-foreground flex items-center gap-1 z-10">
                        <Sparkles className="w-3 h-3" />
                        For You
                      </Badge>
                    )}
                    ...
                    >
                      <img 
                        src={displayVideos[0].thumbnail || videoThumbnails[displayVideos[0].videoId] || `https://img.youtube.com/vi/${displayVideos[0].videoId}/maxresdefault.jpg`} 
                        ...
                        onError={(e) => {
                          ...
                          if (displayVideos[0].videoId && !target.src.includes('placeholder')) {
                            target.src = `https://img.youtube.com/vi/${displayVideos[0].videoId}/hqdefault.jpg`;
                          } else {
                ...
                <div>
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    {displayStories.length > 0 && (displayStories[0] as any)._isPersonalized && (
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-16">
                    {displayStories.map((story, index) => (
                      <div key={story.id} className="p-6 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:shadow-md transition-all duration-200 relative">
                        {(story as any)._isPersonalized && index < 3 && (
                          <Badge className="absolute top-4 right-4 bg-primary text-primary-foreground flex items-center gap-1">
                            <Sparkles className="w-3 h-3" />
                            Recommended
                          </Badge>
                        )}
                ...
                <div>
                  <div className="flex items-center gap-2 mb-6">
                    ...
                    {personalizedAdditionalVideos.length > 0 && personalizedAdditionalVideos[0]._isPersonalized && (
                      <Badge variant="secondary" className="flex items-center gap-1">
                        <Sparkles className="w-3 h-3" />
                        Personalized
                      </Badge>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {personalizedAdditionalVideos.length > 0 
                      ...
                      >
                        {video._isPersonalized && (
                          <Badge className="absolute top-2 right-2 bg-primary text-primary-foreground text-xs flex items-center gap-1">
                            <Sparkles className="w-3 h-3" />
                            For You
                          </Badge>
                        )}
                            ...
                            <img 
                              ...
                              onError={(e) => {
                                ...
                                if (video.videoId && !target.src.includes('placeholder')) {
                                  ...
                                  target.src = "https://via.placeholder.com/320x180/1a1a2e/ffffff?text=Video";
                                }
                              }}
                            />
                            <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              ...
              <div className="xl:col-span-1 space-y-6">
                ...
                <div className="p-6 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:shadow-md transition-all duration-200">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    {personalizedRecommendedVideos.length > 0 && personalizedRecommendedVideos[0]._isPersonalized && (
                            ...
                            <img 
                              ...
                              onError={(e) => {
                                ...
                                if (video.videoId && !target.src.includes('placeholder')) {
                                  ...
                                  target.src = "https://via.placeholder.com/384x216/1a1a2e/ffffff?text=Video";
                                }
                              }}
                            />
                            <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
