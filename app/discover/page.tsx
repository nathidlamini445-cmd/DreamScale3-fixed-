...
/** Stable key for thumbnails / embeds when `videoId` is optional (e.g. some API rows). */
function contentItemVideoKey(v: ContentItem): string {
  return v.videoId ?? v.id
}
...
export default function DiscoverPage() {
  ...
  const userProfile = useMemo(() => {
    // Coerce multi-select fields (industry / businessStage) from string|string[]|null
    // down to a single string|null so the personalization algorithm gets a stable shape.
    const rawIndustry = entrepreneurProfile?.industry
    const industry = Array.isArray(rawIndustry) ? rawIndustry[0] ?? null : rawIndustry ?? null

    const rawStage = entrepreneurProfile?.businessStage
    const businessStage = Array.isArray(rawStage) ? rawStage[0] ?? null : rawStage ?? null
    ...
    const profile = {
      ...
      email: sessionContext?.sessionData?.email || null,
    }
    return profile
      ...
      try {
        ...
        if (data.videos && Array.isArray(data.videos)) {
          ...
          const convertedVideos: ContentItem[] = data.videos.map((v: any, index: number) => {
            const yid = String(v.videoId ?? '').trim() || `yt-fallback-${refreshSeed}-${index}`
            ...
            const thumbnail = v.thumbnail || `https://img.youtube.com/vi/${yid}/hqdefault.jpg`
            
            return {
              id: `yt-${yid}-${refreshSeed}-${index}`, // Make ID unique per refresh
              title: v.title,
              channel: v.channel,
              duration: v.duration,
              views: v.views,
              thumbnail: thumbnail,
              url: v.url || `https://www.youtube.com/watch?v=${yid}`,
              videoId: yid,
              embedUrl: v.embedUrl || `https://www.youtube.com/embed/${yid}`,
          ...
          convertedVideos.forEach(video => {
            const key = contentItemVideoKey(video)
            if (video.thumbnail) thumbnails[key] = video.thumbnail
          })
  ...
  const featuredVideoKey = featuredVideo ? contentItemVideoKey(featuredVideo) : ''
  ...
  const allStories = useMemo(() => {
    const baseStories = industryContent?.entrepreneurStories && industryContent.entrepreneurStories.length > 0
      ? [
          ...industryContent.entrepreneurStories.map(
            (s): ContentItem => ({
              ...s,
              tags: [],
              categories: [],
  ...
  useEffect(() => {
    if (youtubeVideos.length > 0) {
      ...
      youtubeVideos.forEach(video => {
        if (video.thumbnail) {
          thumbnails[contentItemVideoKey(video)] = video.thumbnail
        }
      })
  ...
  const handleVideoClick = (video: ContentItem) => {
    const vid = contentItemVideoKey(video)
    const embedUrl = video.embedUrl || `https://www.youtube.com/embed/${encodeURIComponent(vid)}`
    const url = video.url || `https://www.youtube.com/watch?v=${encodeURIComponent(vid)}`
    setSelectedVideo({ videoId: vid, embedUrl, url })
  }
          ...
          <div className="p-8 space-y-8">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
              ...
              <div className="xl:col-span-3 space-y-8">
                ...
                {/* Featured Video */}
                {!isLoadingVideos && displayVideos.length > 0 && featuredVideo && (
                  <div className="p-6 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:shadow-md transition-all duration-200 relative">
                    ...
                    >
                      <img 
                        src={featuredVideo.thumbnail || videoThumbnails[featuredVideoKey] || `https://img.youtube.com/vi/${featuredVideoKey}/hqdefault.jpg`} 
                        ...
                        onError={(e) => {
                          ...
                          if (featuredVideoKey && !target.src.includes('placeholder')) {
                            if (target.src.includes('hqdefault')) {
                              target.src = `https://img.youtube.com/vi/${featuredVideoKey}/mqdefault.jpg`;
                            } else if (target.src.includes('mqdefault')) {
                              target.src = `https://img.youtube.com/vi/${featuredVideoKey}/sddefault.jpg`;
                            } else if (target.src.includes('sddefault')) {
                              target.src = `https://img.youtube.com/vi/${featuredVideoKey}/maxresdefault.jpg`;
                            } else {
                              // Use a better placeholder with gradient
                              target.src = `https://via.placeholder.com/800x450/6366f1/ffffff?text=${encodeURIComponent(featuredVideo.title.substring(0, 30))}`;
                            }
                          } else {
                            target.src = `https://via.placeholder.com/800x450/6366f1/ffffff?text=${encodeURIComponent(featuredVideo.title.substring(0, 30))}`;
                          }
                    ...
                    <div className="space-y-3">
                      <h2 className="text-2xl font-bold text-foreground">{featuredVideo.title}</h2>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {personalizedAdditionalVideos.length > 0 
                      ...
                      const vk = contentItemVideoKey(video)
                      return (
                      <div 
                        key={video.id} 
                      ...
                      >
                        <div className="flex gap-4">
                          <div className="w-32 h-20 bg-muted rounded-lg overflow-hidden flex-shrink-0 relative group">
                            <img 
                              src={video.thumbnail || videoThumbnails[vk] || `https://img.youtube.com/vi/${vk}/hqdefault.jpg`} 
                              alt={video.title}
                              className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                              loading="lazy"
                              onError={(e) => {
                                ...
                                if (vk && !target.src.includes('placeholder')) {
                                  if (target.src.includes('hqdefault')) {
                                    target.src = `https://img.youtube.com/vi/${vk}/mqdefault.jpg`;
                                  } else if (target.src.includes('mqdefault')) {
                                    target.src = `https://img.youtube.com/vi/${vk}/sddefault.jpg`;
                                  } else if (target.src.includes('sddefault')) {
                                    target.src = `https://img.youtube.com/vi/${vk}/maxresdefault.jpg`;
                                  } else {
                      ...
                      >
                        <div className="flex gap-4">
                          <div className="w-32 h-20 bg-muted rounded-lg overflow-hidden flex-shrink-0 relative group">
                            <img 
                              src={videoThumbnails[vk] || video.thumbnail} 
                              alt={video.title}
                              className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                              onError={(e) => {
                                const target = e.target as HTMLImageElement;
