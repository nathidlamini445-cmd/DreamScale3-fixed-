...
import { VentureQuestCard } from "@/components/venture-quest-card"
import { useSessionSafe } from "@/lib/session-context"
import { AutoSetupWelcomeModal, useAutoSetupWelcome } from "@/components/auto-setup-welcome-modal"

export default function DashboardPage() {
  const [isSettingsModalOpen, setIsSettingsModalOpen] = useState(false)
  const [mounted, setMounted] = useState(false)
  const sessionContext = useSessionSafe()
  const { showWelcome, setupData, handleClose } = useAutoSetupWelcome()
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-foreground relative overflow-hidden">
      ...
      {/* Auto-Setup Welcome Modal */}
      <AutoSetupWelcomeModal
        isOpen={showWelcome}
        onClose={handleClose}
        setupData={setupData}
      />
